...
class Corporate_model extends MY_Sql
{
	...
	public function add_action($post)
	{
		if (isset($post['name']))
		{
			...
			$post['content'] = str_replace('youtube.com/watch?v=','youtube.com/embed/',$post['content']);
			...
			$insert_id = $this -> insert_into('ckeditor', array('name' => $post['name'], 'content' => $post['content'], 'sort' => 0, 'category_id' => $category['category_id'], 'enable' => '1', 'type' => $post['type'], 'lang_type' => $this->session->userdata('lang'),'enable'=>$post['c_status'], 'create_time' => $this->useful->get_now_time(), 'update_time' => $this->useful->get_now_time()));
			return $insert_id;
		}
		else
			return false;
	...
	public function update_action($post, $ck_id)
	{
		if (isset($post['name']))
		{
			...
			$update_id = $this -> update_set('ckeditor', array('ck_id' => $ck_id), array('category_id' => $category['category_id'], 'name' => $post['name'], 'content' => $post['content'],'enable'=>$post['c_status'], 'update_time' => $this->useful->get_now_time()));
			return $update_id;
		}
		else
			return false;
