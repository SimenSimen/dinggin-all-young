...
class Member_model extends MY_Model
{
  ...
  /** normal member */
  const BUYER_ROLE_NORMAL = 0;

  /** member who can sale */
  const BUYER_ROLE_SALE = 1;

  /** wait for validating */
  const BUYER_ROLE_PENDING = 2;
  ...
  {
    $sql = "select b.by_id,b.d_account,b.PID,b.name,if(b.sex='male','男','女') as sex,m.upline,b.create_time as ctime,b.mobile,b.d_is_member,m.member_num,m.deadline,b.d_dividend,b.d_shopping_money";
    $sql .= " from buyer b";
    $sql .= " left join member m on m.by_id=b.by_id";
    $sql .= " left join member m1 on m1.member_id=m.upline";
    $sql .= " left join buyer b1 on b.PID=b1.by_id";
    $sql .= " left join buyer b2 on m1.by_id=b2.by_id";
    $sql .= $where;
    $sql .= ' order by b.create_time desc';
    $sql .= $page;
  ...
  public function GetVip($where = '', $page = '')
  {
    $sql = "select b.by_id,b.name,m.member_num,m.is_family_boss,";
    $sql .= " (SELECT count(*) FROM `buyer` b1 WHERE b1.PID=b.by_id AND d_is_member=0) count_normal_buyer,";
    $sql .= " (SELECT count(*) FROM `buyer` b2 WHERE b2.PID=b.by_id AND d_is_member=1) count_vip_buyer";
    $sql .= " FROM buyer b";
    $sql .= " LEFT JOIN member m on m.by_id=b.by_id";
    $sql .= $where;
    $sql .= ' order by b.create_time desc';
    ...
    $query = $this->db->query($sql)->result_array();
    foreach ($query as $key => $value) {
      $query[$key]['is_family_boss'] = ($value['is_family_boss'] == 'Y') ? '是' : '否';
    }
   ...
   * 取得推薦人資訊
   */
  public function getRecommendPerson($pid)
  {
    return $this->db->query('SELECT * FROM buyer WHERE by_id = ?', $pid)->result_array()[0];
    /*
		# 可取得組織表之SQL語句
		SELECT m.member_num, a.name, f.d_name, m.is_family_boss, m.join_time, b.name
		FROM buyer a
		LEFT JOIN buyer b
		ON a.PID = b.by_id
		INNER JOIN member m
  ...
  {
    return $this->db->query('SELECT * FROM buyer WHERE by_id = ?', $id)->result_array()[0];
  }
