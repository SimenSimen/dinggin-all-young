<?php
class Gemail_model extends MY_Model {
    ...
    public function select_timer_between($table, $member_id, $initial, $deadline, $data_where='')
    {
        $sql = '
            ...
            FROM    `'.$table.'`
        ';
        /*目前為了讓功能正常，目前先讓所有資料出現。刪除$initial跟deadline時間區段的查詢*/
        $sql .= '
            WHERE   (   
                        `type` = 1
                        AND     `member_id`    = "' .$member_id.'"
    ...
    public function get_member_name($member_array = array())
    {
        if (!empty($member_array))
        {
            ...
            {
                $iqr = $this -> select_from('buyer', array('by_id' => $value));
                if ($count % 5 == 0)
                {
                    $array_key++;
                    $name_array[$array_key][] = $iqr['name'];
                }
                else {
                    $name_array[$array_key][] = $iqr['name'];
                }
		...
		//發mail
	public function send_mail($gemail,$member_array = array(),$host){
		if(empty($gemail)){
			return;
		}
		if (!empty($member_array))
		{
				foreach ($member_array as $key => $value)
				{
						$iqr = $this -> select_from('buyer', array('by_id' => $value));
						// 寄信
						$this->mod_index->send_mail($host['domain'], $this->lang_menu['jcymall'], $iqr["by_email"], $gemail['d_title'], $gemail['d_message']);
				}
		}
		else
		{
