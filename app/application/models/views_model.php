...
class Views_model extends MY_Sql
{
	...
	public function category_list_first()
	{
		...
		foreach ($data['list_news'] as $key => $value) {
			$content_data = $this -> select_from_order_limit('enews', array('category_id'), array('category_id' => $value['id'], 'lang_type' => $this -> set_language, 'enable' => '1'), 'sort, update_time', 'ASC');
			if(empty($content_data)){
				unset($data['list_news'][$key]);
			}
		}
		$data['list_news']=array_values($data['list_news']);
		$data['list_aesthetic'] = $this -> select_from_order_limit('auth_category', array('category_id as id', 'c_name as name', 'sort'), array('type' => '3', 'lang_type' => $this -> set_language), 'sort, update_time', 'ASC');
		foreach ($data['list_aesthetic'] as $key => $value) {
			$content_data = $this -> select_from_order_limit('enews', array('category_id'), array('category_id' => $value['id'], 'lang_type' => $this -> set_language, 'enable' => '1'), 'sort, update_time', 'ASC');
			if(empty($content_data)){
				unset($data['list_aesthetic'][$key]);
			}
		}
		$data['list_aesthetic']=array_values($data['list_aesthetic']);
		...
		$data['list_photo_type'] = $this -> select_from_order_limit('photo_type', array('d_id as id', 'd_name as name'), array('d_enable' => 'Y', 'lang_type' => $this->set_language), 'd_sort', 'ASC');
	...
	public function category_list($element,$element_id=0)
	{
		$data['filename'] = 'list';
		...
		switch ($element) {
			...
			case 'aesthetic':
				$data['title'] = $lang['News'];
				$data['list'] = $this -> select_from_order_limit('auth_category', array('category_id as id', 'c_name as name', 'sort'), array('type' => '3', 'lang_type' => $this -> set_language), 'sort, update_time', 'ASC');
				foreach ($data['list'] as $key => $value) {
					$content_data = $this -> select_from_order_limit('enews', array('category_id'), array('category_id' => $value['id'], 'lang_type' => $this -> set_language, 'enable' => '1'), 'sort, update_time', 'ASC');
					if(empty($content_data)){
						unset($data['list'][$key]);
			...
			case 'photo':
				...
				$data['list'] = $this -> select_from_order_limit('photo_category', array('d_id as id', 'd_name as name', 'd_photo'), array('d_enable' => 'Y', 'd_photo <>' => '', 'lang_type' => $this->set_language, 'd_type' => $element_id), 'd_sort, d_updatetime', 'ASC');
				foreach ($data['list'] as $key => $value)
				{
