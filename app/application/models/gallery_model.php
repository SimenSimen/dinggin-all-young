...
class Gallery_model extends My_Sql
{
	...
	public function stretch($type, $id)
	{
		switch ($type) {
			...
			case 't':
				$array = $this -> select_from('photo_type', array('d_id as id', 'd_name as name', 'd_enable', 'd_content'), array('d_id' => $id));
				break;

			case 'g':
				$array = $this -> select_from('photo_category', array('d_id as id', 'd_name as name', 'd_enable', 'd_content', 'd_video_link', 'd_video_name', 'd_type'), array('d_id' => $id));
				break;
			
			case 'c':
				$array = $this -> select_from('images', array('img_id as id', 'img_note as name','enable as img_status'), array('img_id' => $id));
				break;
		}
	...
	public function pic_status($table){
    	$sql="SELECT `img_id`,`img_path` FROM $table where enable = '1' ";
