...
class Gold_model extends MY_Model
{
	 ...
	 * @return array
	 */
	public function pageDividend($where = [], $page = 1, $pageSize = 10)
	{
		$this->db->from('dividend')->where($where);
	...
	/**
	 * Calculate the expired point by date
	 *
	 * @param string $expireDate
	 * @param string $buyer
	 * @return float
	 */
	public function dividendExpire($expireDate, $buyer)
	{
		$this->db
			->select('COALESCE(SUM(d_val),0) as points', false)
			->from('dividend')
			->where('send_dt is not null')
			->where("DATE_ADD(send_dt, INTERVAL 365 DAY) <= '{$expireDate}'")
			->where(['d_type' => 19, 'buyer_id' => $buyer, 'is_del' => 'N']);

		$data = $this->db->get()->result_array();

		$result = floatval(@$data[0]['points'] ? $data[0]['points'] : $data[0]['points']);
		return $result;
	}
}
