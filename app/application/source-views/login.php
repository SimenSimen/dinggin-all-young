...
<body>
  ...
  <div class="container" style="width:360px;">
    ...
    <form class="form-signin" name='signin_form' id='signin_form' action="/index/login" method="post">
    	...
    	<!--登入帳號密碼欄位-->
  		<input type="text" 		placeholder="<?=$AccountNumber?>"	name='account' 	id='account' class="input-block-level" maxlength="32">
      ...
      <p>
        <select id="lang">
          <option value="zh-tw" <?php if($lang == 'zh-tw'):?>selected<?php endif; ?>><?=$Zh_tw?></option>
          <option value="zh-cn" <?php if($lang == 'zh-cn'):?>selected<?php endif; ?>><?=$Zh_cn?></option>
          <option value="english" <?php if($lang == 'english'):?>selected<?php endif; ?>><?=$English?></option>
        </select>
        <script>
      ...
      <?php if ($host['domain'] != 'multi-level.appplus.com.tw'): ?>
