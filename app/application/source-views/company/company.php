				...
				*/
				...
				if($this->input->post('card_owner') != $admin['member_id'])
				{
					...
					$message = ''.
						...
						"<p>詳細情況：{unwrap}<a href='http://".$member_domain['domain']."/business/uform_sign_up_show/sign_up_".$uform['ufm_id']."/".$this->input->post('card_owner')."'>報名情形總覽</a>{/unwrap}</p>".
						"<hr>".
				...
				$message = ''.
					...
					"<p>報名表單：".$uform['ufm_name']."</p>".
					"<p>詳細情況：{unwrap}<a href='http://".$member_domain['domain']."/business/uform_sign_up_show/sign_up_".$uform['ufm_id']."/".$admin['member_id']."'>報名情形總覽</a>{/unwrap}</p>".
					"<hr>".
					"<p>注意：本郵件是由系統自動產生與發送，請勿直接回覆。</p>";
				//寄信
