   ...
   */?>
   ...
   <div class="step-imgUP-1">
      ...
      <?php if ($photo_num[$d_id] == 0): ?>
        <h2><?php echo $val["d_name"];?>：目前沒有任何照片</h2>
      <?php else: ?>
        <h2><?php echo $val["d_name"];?> (共<?=$photo_num[$d_id]?>張)</h2>
      <?php endif; ?>
      <input type='button' class="aa2" id='add_newphoto' d_id='<?php echo $d_id;?>' value='+ 新增照片'><?php if($pnum==1):?><span class='introdution_prompt' id='msg'><?=$msg?></span><?php endif;?>
      ...
      <div class="imgUPss">
              ...
              <table style="width:100%;margin-left: 10px;margin-top: 10px;">
                <?php foreach ($photo[$d_id] as $key => $value): ?>
                  <tr>
                    <td class='edit_photo_box'><img src='<?=$value?>'></td>
                    <td>
                      <input value='<?=$photo_name[$d_id][$key]?>' class='edit_photo_name_area iii4' name='photo_name[<?=$photo_id[$d_id][$key]?>]' placeholder='照片註解(最多64個字)' maxlength='64'>
                    </td>
                  </tr>
                <?php endforeach; ?>
              </table>
        ...
        <div class='switch_newphoto' id='newphoto_delete' d_id="<?php echo $d_id;?>" style="display:none;">
          <span class='introdution_prompt'>請勾選您要從相簿移除的照片</span><input name="clickAll3<?php echo $d_id;?>" id="clickAll3<?php echo $d_id;?>" d_id="<?php echo $d_id;?>" type="checkbox"><label for='clickAll3<?php echo $d_id;?>'>全選</label>
      ...
      <div class="aaCenter">
        <div class="bottom_button" id='newphoto_bottom_button' d_id="<?php echo $d_id;?>">
          <input type='button' class='aa6' id='newphoto_sort_btn' d_id="<?php echo $d_id;?>" value='排序照片'>
          <input type='button' class='aa6' id='newphoto_edit_btn' d_id="<?php echo $d_id;?>" value='編輯註解'>
          <input type='button' class='aa6' id='newphoto_del_btn' d_id="<?php echo $d_id;?>" value='移除照片'>
        </div>
        <div class="bottom_button" id='newphoto_sort_button' d_id="<?php echo $d_id;?>" style="display:none;">
            <input type='button' class='aa6' id='newphoto_sort_submit' d_id="<?php echo $d_id;?>" value='儲存排序'>
            <input type='button' class='aa6 newphoto_cancle_btn' d_id="<?php echo $d_id;?>" value='取消'>
        </div>
        <div class="bottom_button" id='newphoto_edit_note_button' d_id="<?php echo $d_id;?>" style="display:none;">
            <input type='button' class='aa6' id='newphoto_edit_note_submit' d_id="<?php echo $d_id;?>" value='儲存照片註解'>
            <input type='button' class='aa6 newphoto_cancle_btn' d_id="<?php echo $d_id;?>" value='取消'>
        </div>
        <div class="bottom_button" id='newphoto_remove_button' d_id="<?php echo $d_id;?>" style="display:none;">
            <input type='button' class='aa6' id='newphoto_remove_submit' d_id="<?php echo $d_id;?>" value='移除所選照片'>
            <input type='button' class='aa6 newphoto_cancle_btn' d_id="<?php echo $d_id;?>" value='取消'>
        </div>
      </div>
