...
<html lang="en" class="no-js">
     ...
     <div class="www">
      <div class="row ">
           ...
           <div class="col-xs-2 text-right">
                ...
                data-content="<?=$GoodsHaveBeen?><br><span style='color:#DA5049'>0</span> <?=$Item?><br><?=$TotalAmount?>(TWD)<br><span style='color:#DA5049'>0</span> <?=$Yuan?>" >
  ...
  <div class="main">
    ...
    <div class="detail-L">
           <div class="img-size">
              <!--<img src="tem/entIMG/240.jpg" alt=""/> 寬度240 圖片置中-->
       ...
       <div class="detail-R"> 
           <div class="row">
               ...
               <div class="col-lg-2 col-md-3 col-xs-4"><?=$SuggestedRetailPrice?></div>
               <?php if ($prd['prd_price01'] != ''): ?>
               <div class="col-lg-10  col-md-9  col-xs-8">$ <?=$prd['prd_price01']?></div>
               <?php else: ?>
               <div class="col-lg-10  col-md-9  col-xs-8"><?=$No?></div>
               <?php endif; ?>

               <div class="col-lg-2 col-md-3  col-xs-4"><?=$SpecialOffer?></div>
               <div class="col-lg-10  col-md-9 col-xs-8"><mark>$ <?=$prd['prd_price00']?></mark></div>
               <?php if ($prd['prd_active'] == 0): ?>
                <div class="col-lg-2 col-md-3  col-xs-4"><?=$StorageCapacity?></div>
                <div class="col-lg-10  col-md-9 col-xs-8"><?=$prd['prd_amount']?>   &nbsp;
                <a id="prd_amount" href="#prd_amount_div"><?=$ReferenceOnly?></a>
                  <div id="prd_amount_div" style="display: none;"><?=$ActualNumberGoods?></div>
                  </div>
               <?php endif; ?>

               <div class="col-lg-2 col-md-3  col-xs-4"><?=$PaymentMethod?></div>
                ...
                <?php endif;?>
               <div class="col-lg-2 col-md-3  col-xs-4"><?=$DeliveryMethod?></div>
                <?php if(empty($logistics_way)): ?>
                  <div class="col-lg-10  col-md-9 col-xs-8"><?=$No?> &nbsp;</div>
                <?php else: ?>
                  <?php foreach ($logistics_way as $log_key => $log_value): ?>
                    <div class="col-lg-10  col-md-9 col-xs-8" style="float: right;"><img style="width: 30px;height: auto; vertical-align: middle;margin-right:5px;" src="<?=$log_value['lway_image']?>"><?=$log_value['lway_name']?> <?=$log_value['fee']?></div>
                  <?php endforeach; ?>
                <?php endif; ?>
                <?php if(!empty($fee)):?>
                  <div class="col-lg-10 col-md-9  col-xs-8" style="float: right;"><?=$Purchase?> <span style="color:red;"><?=$fee?></span> 元，<span style="color:red;">免運費</span></div>
                <?php endif; ?>
      </div> 
      ...
      <div class="row">
               ...
               <div class="col-lg-8 col-md-9 col-sm-8 col-xs-7">
                <button class="aa4" id="add_to_cart"><?=$AddCart?><span class="icon-cart32"></span></button>
       ...
       <div class="tabs">
                <ul class="tab-links">
                    <li class="active"><a href="#buy1" ><?=$ProductDesciption?></a></li>
                    <li><a href="#buy2"><?=$ProductSpecifications?></a></li>
                    <li><a href="#buy3"><?=$BuyExplanation?></a></li>
                    <li><a href="#buy4"><?=$TransportRules?></a></li>
                </ul>
...
<!--2商品規格-->
                  <div id="buy2" class="tab">
                    <?php if (!empty($prd_specification_content)): ?>
                      ...
                      <?php endforeach; ?>
                      <?php else: ?>
                        <p><?=$NoProductSpecifications?></p>
                    <?php endif; ?>
<!-- 保固範圍-->
                    <?php if ($prd['prd_assurance_range'] != '' || $prd['prd_assurance_date'] != ''): ?>                    
                      
                      <?php if ($prd['prd_assurance_range'] != ''): ?>
                        <p><?=$_WarrantyCoverage?><?=$prd['prd_assurance_range']?></p><br/>
                      <?php endif; ?>
                      
                      <?php if ($prd['prd_assurance_date'] != ''): ?>
                        <p><?=$_WarrantyPeriod?><?=$prd['prd_assurance_date']?></p><br/>
                      <?php endif; ?>
...
<!--3購買說明-->
                  ...
                  <p class="prd_content"><strong><?=$ShoppingGuide?></strong><br><br>
...
<!--4運送規則-->
  ...
  <?php if($store['cset_share_btn']): ?>
    <section class="www" id="share"> 
      ...
      <a class="shareTo sina" href="javascript:(function(){window.open('http://v.t.sina.com.cn/share/share.php?title='+encodeURIComponent(document.title)+'&url='+encodeURIComponent(location.href)+'&source=bookmark','_blank','width=450,height=400');})()"  title="<?=$SinaWeibo?>" ><span class="icon-weibo"></span></a>
      <!-- <a class="shareTo weChat" href="#"  title="WeChat 微信"><span class="icon-wechat"></span></a> -->
      <a class="shareTo plurk" href="javascript: void(window.open('http://www.plurk.com/?qualifier=shares&status=' .concat(encodeURIComponent(location.href)) .concat(' ') .concat('&#40;') .concat(encodeURIComponent(document.title)) .concat('&#41;')));" title="<?=$Plurk?>"></a>
      ...
      <a class="shareTo qq" href="javascript: void(window.open('http://share.v.t.qq.com/index.php?c=share&a=index&url='.concat(encodeURIComponent(location.href)) +'&title=' .concat(encodeURIComponent(document.title)) ));"  title="<?=$TencentWeibo?>"><span class="icon-tencent-weibo"></span></a>
      <a class="shareTo ren" href="javascript: void(window.open('http://widget.renren.com/dialog/share?resourceUrl='.concat(encodeURIComponent(location.href)) +'&title=' .concat(encodeURIComponent(document.title)) ));"  title="<?=$Everyone?>"><span class="icon-renren2"></span></a>   
