...
class member_register extends MY_Controller
{
	...
	public function register ()
	{
		...
		if (! empty($buyer_data)) {
			$this->useful->AlertPage('/member_register_on/' . Comment::SetValue('PID'),$this->lang1['someacc'] /* 已有相同帳號，請重新輸入? */);
			return '';
		}
		...
		if(!empty($this->_check->main())){
			echo $this->_check->main(base_url('member_register_on'));
			return '';
		}
	...
	public function sms_code(){
		...
		$this->load->view('index/header'.$this->style, $data);
		$this->load->view('index/member/member_register1', $data);
		$this->load->view('index/footer'.$this->style, $data);
	}
...
*/
		...
		$this->load->view('index/header'.$this->style, $data);
		$this->load->view('index/member/member_nav', $data);
		$this->load->view('index/member/member_register_ok', $data);
		$this->load->view('index/footer'.$this->style, $data);
	}
}