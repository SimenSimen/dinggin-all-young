...
class push extends MY_Controller
{
    ...
    public function registration()
    {
        ...
        $member_id    = 1;//$this -> input -> post('member_id');
        $baidu_appid  = $this -> input -> post('baidu_appid');
        ...
        if($uniqued != '')
        {
            ...
            if(!empty($device_data))
            {
                ...
                if(!empty($device_token) && (($device_data['device_token'] != $device_token))) // 修改
                {
                    $update_data = array(
                ...
                else if (!empty($baidu_appid) && (($device_data['baidu_appid'] != $baidu_appid))){
                     $update_data = array(
                        'app_version'   => $this -> input -> post('appversion'),
                        'model'         => $this -> input -> post('devicemodel'),
                        'version'       => $this -> input -> post('deviceversion'),
                        ...
                        //百度
                        'baidu_appid'     => $this -> input -> post('baidu_appid'),
                        'baidu_channelId' => $this -> input -> post('baidu_channelId'),
                        'baidu_requestId' => $this -> input -> post('baidu_requestId'),
                        'baidu_userId'    => $this -> input -> post('baidu_userId')
                    );
                    $this -> mod_push -> update_set('push_device', 'id', $device_data['id'], $update_data);
                    $result = array('result'=> 1, 'note'=> 'update success');
                }
    ...
    public function add()
    {
        ...
        else
        {
            ...
            if(!$this -> input -> post('add'))
            {
                ...
                if($data['group_push'])
                {
                    $members = $this -> mod_push -> select_from_order('member', 'member_id', 'asc', array('domain_id' => $this -> domain_id,'member_id' => $this -> member_id));
    	...
    	if($p_id != "")
    	{
	    	...
	    	if(!empty($to_member_array))
	    	{
		    	...
		    	foreach ($to_member_array as $key => $value)
		    	{
                    ...
                    $gcm_key = ($to_member['gcm_key']) ? $to_member['gcm_key'] : "0";                    
                    $sys_push=$to_member['sys_push'];
                    $apikey=$to_member['apikey'];
                    $secretkey=$to_member['secretkey'];
		    		//$pem_key = $_SERVER['DOCUMENT_ROOT'] . $to_member['pem'];
...
*/
    public function group_push($p_id)
    {
        if($p_id != "")
        {
            $push_log = $this -> mod_push -> select_from('push_log', array('p_id' => $p_id, 'type' => '0'));
            ...
            {     
                ...
                foreach ($to_member_array as $key => $value)
                {
                    ...
                    $secretkey=$to_member['secretkey'];
                    if($sys_push==1){
                        $fp = popen("php ".dirname(__FILE__)."/robot_baidu.php $gcm_key $member_id $pem_key $writer $p_id $apikey $secretkey $sys_push> /dev/null &",'r');
                    }else{
                        $fp = popen("php ".dirname(__FILE__)."/robot.php $gcm_key $member_id $pem_key $writer $p_id > /dev/null &",'r');
                    }
                    // read popen
    ...
    {
        $member_id = 1;//$this -> input -> get('member_id');
        // App_Timer 當前開啟時間
