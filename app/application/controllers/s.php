...
class S extends MY_Controller
{
	...
	public function ajaxallpackagename()//後台APP憑證
	{
		$sys_push=$_POST['sys_push'];
		$member_id=$_POST['member_id'];
		$sql='select member_id,account from member where member_id=?';	
		$member = $this->db->query($sql, array($member_id))->row_array();
		
		echo json_encode(array('packagename'=>$this->allpackagename($member['member_id'],$member['account'],$sys_push)));
	}
}