...
{	
	...
	public function index(){
		...
		提醒您，naturefa系統新增待處理通知如下，<br><br>

		請於後台登入您的管理帳密進行處理。<br>
		naturefa後台：<a href="http://naturefa.com/index/login" style="font-family:Verdana;font-size:90%" target="_blank">naturefa後台</a><br><br><br>
		<div style="width:1000px;padding:0; margin:0; font-family:"微軟正黑體", "新細明體", "標楷體", Arial; font-size:.8em; display:block; text-align:right; font-size:1.1em;">
			<table width="0" border="0" cellpadding="0" cellspacing="0" style="width:100%;color:rgb(0,0,0);font-size:0.9em;display:block;padding:10px 5px">
 			<tbody>
 			<tr>
 				<th colspan="4" style="padding:8px;color:rgb(255,255,255);background:rgb(122,161,73)">naturefa系統通知</th>
 			</tr>';
        if(!empty($order)):
 		$message.='
 			<tr>
		...
		if(!empty($order) or !empty($atmnum) or !empty($memnum) or !empty($connum)): 
			...
			foreach ($mail as $key => $value) {
			 	$this->mod_index->send_mail('service@i-qrcode.com', $this->data['web_config']['title'], $value, 'naturefa管理者系統通知', $message);	
			}
		endif;
		// 記LOG
		$this->WriteLog();
	...
	private function SendMember(){
		...
		foreach ($dbdata as $key => $value) {
			...
			提醒您，您在naturefa系統-會員即將過期，請點下列連結進行會員續約<br><br>
			...
			$this->mod_index->send_mail('service@i-qrcode.com', $this->data['web_config']['title'], $value['email'], 'naturefa管理者系統通知', $message);	
		}
	}
	// 寫入LOG
