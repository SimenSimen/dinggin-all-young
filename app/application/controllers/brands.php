...
class Brands extends MY_Controller
{
	...
	public function index($pid=0){
		...
		$prd_class=$this->mymodel->select_page_form($dbname_class,'','prd_cid',array('prd_cid'=>$pid,'d_enable'=>'Y'));
		if(!empty($product_class) and $pid<>0){
			$this->useful->AlertPage('/products/'.$product_class['0']['prd_cid']);
		}else if(empty($prd_class)){
	...
	//購物車
	public function ajax_car(){//寫入購物車
		if($_SESSION['MT']['is_login']==1){
	...
	public function ajax_search(){//前台讀取產品分類
	...
	public function ajax_del_search(){//清除搜尋紀錄
	...
	public function brands_list(){
		...
		//分頁程式 start
		$data['ToPage']=$Topage=!empty($_POST['ToPage'])?$_POST['ToPage']:1;
		$qpage=$this->useful->SetPage_0($dbname,'',20,array('d_enable'=>'Y', 'lang_type'=>$this -> session -> userdata('lang')));
		...
		//第一層分類
		$data['dbdata']=$dbdata=$this->mymodel->select_page_form_0($dbname,$qpage['result'],'*',array('d_enable'=>'Y', 'lang_type'=>$this -> session -> userdata('lang')),'prd_cid');
		//第二層分類
		foreach ($dbdata as $key => $value) {
			$prd_cid 					=	$value['prd_cid'];
			$data['data_sub'][$prd_cid]	=	$this->mymodel->select_page_form($dbname,'','*',array('d_enable'=>'Y', 'lang_type'=>$this -> session -> userdata('lang')),'prd_cid');
		}
		//檔案名
			...
			if($dbname=='products'){
				...
				if($_FILES['prd_image']['error']['0'] != 4)
				{
					...
					foreach ($img as $key => $value) {
						if(!empty($value['path'])){$img_path[]=$value['path'];}
					...
					if(!empty($img_path)){$_POST['prd_image']=implode(',',$img_path);}
					if($id){
