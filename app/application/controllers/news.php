...
class News extends MY_Controller
{
		...
		if(!$this->session->userdata('member_id'))
		{
			...
			switch ($opr) {
				...
				case 'delete':				
					...
					$this -> script_message('刪除成功', '/news/main/' . $this -> input -> post('type') );
					break;
	...
	public function article_status()
	{
		...
		if(is_array($ck_array) && is_array($del_array) && $Art_status != '#')
		{
			...
			$this -> script_message('更改成功', '/news/main/' . $type);
		}
		else
			$this -> script_message('請選擇筆數或狀態', '/news/main/' . $type);
