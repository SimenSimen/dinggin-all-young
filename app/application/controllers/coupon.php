...
class Coupon extends MY_Controller
{
	...
	public function main()
	{
		...
		foreach($data['coupon'] as $key => $val){
			$data['coupon'][$key]['enable'] = ($val['enable'] == 0)?'隱藏中':'公開中';
		}
						...
						switch ($share_mode) {
							case 1:
								...
								$insert_data = array(
									...
									'enable'	=> $this->input->post('coup_enable')
								);
	...
	public function article_status()
	{
		...
		if(is_array($ck_array) && is_array($del_array) && $Art_status != '#')
		{
			...
			foreach ($del_data as $key => $value)
			{
				$update_id[] = $this -> mod_coupon -> update_set('ecoupon', array('ecp_id' => $value), array('enable' => $Art_status ));
			}
			$this -> script_message('更改成功', '/coupon/main/' . $type);
		}
		else
			$this -> script_message('請選擇筆數或狀態', '/coupon/main/' . $type);
