...
class Order_model extends MY_Model {
	...
	public function orderList($by_id,$page_limit,$Topage,$orderNumber,$date){
    	...
    	$sql="SELECT o.`id`, o.`order_id`, o.`create_time`, o.`status`,o.`product_flow`,o.`deadline`
	...
	public function orderdata($id){
    	...
    	$sql="SELECT o.`order_id`, o.`member_id`, o.`name`, o.`phone`, o.`address`, o.`pay_way_id`, o.`lway_price`, o.`lway_id`, o.`status`, o.`pay_price`, o.`total_price`, o.`price_money`, o.`use_dividend`, o.`atmno`, o.`atmdate`, o.`trade_date`, o.`shop_id`, o.`total_lv`, o.`back_name`, o.`back_bank`, o.`back_account`, o.`back_note`, o.`apply_back_date`, o.`product_flow`,o.`bonus`,o.`create_time`,o.`country`,o.`county`,o.`area`,o.`deadline`,o.`atmpayment`
	...
	public function get_order_sale_excel($where=""){//匯出出貨明細報表
		...
		$title_array=array("prd_name"=>"產品項目","number"=>"銷量","price"=>"產品單價","total_price"=>"銷售額","total_count"=>"下單數");
		$i=1;
	...
	public function get_order_back_excel($where=""){//匯出退貨明細報表
		...
		$title_array=array("prd_name"=>"產品項目","number"=>"數量","price"=>"產品單價","total_price"=>"退款金額","total_count"=>"下單數");
		$i=1;
		$objPHPExcel->getActiveSheet()->setCellValue('A'.($i), '退貨明細');
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$i.':'.$word[(count($title_array))].$i);//合併
		$objPHPExcel->getActiveSheet()->getStyle('A'.($i))->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);//置中
	...
	public function get_order_excel($where=""){//匯出訂購單明細報表
		...
		if(!empty($order_id_array)){
			$sql="select * from order_details where oid in(".implode(",",$order_id_array).") group by oid";
			$query = $this->db->query($sql);
	...
	public function get_order_back_data($where="",$page=''){//抓取退貨訂單資料*
		$data=array();
		$sql="select od.prd_id,od.prd_name,sum(od.number) as number,(od.price) as price,sum(od.total_price) as total_price,count(*) as total_count 
			from `order` o
			inner join `order_details` od on o.id=od.oid ".$where." group by od.prd_id,od.price";
	...
	public function get_order_sale_data($where="",$page=''){//抓取出貨訂單資料*
		$data=array();
		$sql="select prd_id,prd_name,sum(number) as number,price,sum(total_price) as total_price,count(*) as total_count from order_details ".$where." group by prd_id,price";
		if(!empty($_SESSION["AT"]["where"]["sort"])){
			if($_SESSION["AT"]["where"]["sort"]=="total_count"){
				$sql.=" order by count(*) ".$_SESSION["AT"]["where"]["sort_ad"];
			}elseif($_SESSION["AT"]["where"]["sort"]=="number"){
	...
	public function get_order_supplier_data($where="",$page=''){//抓取銷貨(供應商)訂單資料*20171228
		$data=array();
		$sql="select o.prd_id,o.prd_name,sum(o.number) as number,o.price,sum(o.total_price) as total_price,count(*) as total_count, o.supplier_id ,s.d_name
			from order_details as o
			inner join supplier s on s.d_id=o.supplier_id
			".$where." group by o.prd_id,o.price";
	...
	public function get_order_data($where="",$page=''){//抓取訂單資料*
		...
		inner join warehouse w on w.d_id=o.warehouse_id ".$where." group by o.order_id order by o.id Desc";
		$sql.=$page;
		$query = $this->db->query($sql);
		$by_id=array();
		foreach($query->result_array() as $key=>$val){
	...
	public function get_order_details_data($id=''){ //抓取詳細訂單資料
		$where=(!empty($id))?$id:"0";
		$sql="select id,prd_name,price,number,total_price from order_details where oid=".$where." order by id asc";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	...
	public function send_mail($id,$host){
		...
		$subject = $host['company'].$language['OrderNumber'].':'.$dbdata['order_id'].'【'.$product_flow_txt.'】 - ' .$this -> web_config['title'];
    ...
    <td width="25%" style="border-bottom:1px #CCC solid; padding:8px 5px;text-align:left;background: #f1efef;"><strong>'.$language['Subtotal'].'</strong></td>
    	...
    	$payment_way_txt=isset($payment_way[$dbdata['pay_way_id']])?$payment_way[$dbdata['pay_way_id']]:"";
 		$message.='<tr><td colspan="3" style="width:100%;border-bottom:1px #CCC solid; padding:8px 5px;">
 				...
 				<span>'.$language['TotalPrice'].number_format($dbdata['total_price']).'</b> '.$language['NTDollars'].'</span></div></td></tr>
</table>
<table width="0" border="0" cellpadding="0" cellspacing="0" id="info" style="width:100%; color:#000; padding:5px;font-size:.9em; display:block; padding:10px 5px; ">
  <tr>
  ...
  <tr>
    ...
    <td colspan="3" style="border-bottom:1px #CCC solid; padding:8px 5px;"><div><b style="color:#F60; display:block;">'.$payment_way_txt.'</b>';
    	...
    	$message.='</div></td></tr>
	  <tr>
	    <td style="border-bottom:1px #CCC solid; padding:8px 5px;text-align:center;background: #f1efef;"><strong>'.$language['other'].'</strong></td>
	    <td colspan="3" style="border-bottom:1px #CCC solid; padding:8px 5px;">'.$language['willcancel'].'</td>
	  </tr>
	</table>
	<div style="font-size:.9em; display:block; padding:10px 5px; font-style:italic;">'.$language['DoNotDirectlyReply'].'</div></div>';

		// 寄信
		$this->mod_index->send_mail($host['domain'], $host['company'], $dbdata["email"], $subject, $message);//寄會員
		if($dbdata["email"]!=$dbdata["email_buy"]){
			$this->mod_index->send_mail($host['domain'], $host['company'], $dbdata["email_buy"], $subject, $message);//寄購買者
		}
	}
