...
class products_model extends CI_Model{
	...
	public function productsList($pid,$page_limit,$Topage,$d_spec_type){
		if($d_spec_type==1){
			$sql_where ="where p.`prd_active`='1' and p.`lang_type`='$this->setlang' and p.`is_bonus`='N'";
		}else{
			$sql_where ="where p.`prd_active`='1' and p.`lang_type`='$this->setlang' and p.`is_vip`='N' and p.`is_bonus`='N'";
		}
		if($pid!=0){
	...
	public function productsDetail($id,$d_spec_type){
		...
		if($d_spec_type==1){
			$sql_where ="where p.`prd_id`=$id and p.`lang_type` ='$this->setlang' and p.`prd_active`='1' and p.`is_bonus`='N'";
		}else{
			$sql_where ="where p.`prd_id`=$id and p.`lang_type` ='$this->setlang' and p.`prd_active`='1' and p.`is_vip`='N' and p.`is_bonus`='N'";
		}
