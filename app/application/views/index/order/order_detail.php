                <section class="content has-side">
                    <div class="title"><?=$this->lang_menu['order'];//個人訂單查詢?></div>   
                      ...
                      <div class="order-detail">
                        ...
                        <?if($orderdata['pay_way_id']==9){?>
                        <table class="table table-v">
                            <tbody>
                                <tr>
                                    <th><?=$orderdata['payment_way'];//支付寶?><br><?=$this->lang['ordernumber'];//訂單編號?></th>
                                    <td><strong><?=$orderdata['alipay_id'];?></strong></td>
                                </tr>
                            </tbody>
                        </table>
                        <?}?>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                         ...
                                         <tr>
                                            ...
                                            <td><?=$orderdata['status_name'];?>
                                                <?if($orderdata['status']==0 && $orderdata['pay_way_id']==9){?><a href="/alipay/pay/repay/<?=$id;?>">重新付款</a><?}?>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th><?=$this->lang['o_60'];//配送方式?></th>
                                            <td>
																							<?=$orderdata['logistics_way'];?><?=$shop_address;?>
																							<?php if(!empty($orderdata['tracking_name']) and !empty($orderdata['tracking_num'])){ ?>
																							<br>
																							<?=$orderdata['tracking_name'];?>(<?=$orderdata['tracking_num'];?>)
																							<?php } ?>
																						</td>
                                        </tr>
                                        <tr>
                                            <th><?=$this->lang['buyer_note'];//備註?></th>
                                            <td><?=$orderdata['buyer_note'];?></td>
                                        </tr>
                                        <? if ($orderdata['status']<>0 && (!empty($orderdata['atmno'])) && $orderdata['pay_way_id']==4){?>
                                            <tr>
                                                <th><?=$this->lang['o_52'];//匯款後五碼?></th>
                                        ...
                                        <? if ($orderdata['status']==4){?>
                                            <tr>
                                                <th><?=$this->lang['cancel_reason'];//取消原因;?></th>
                                                <td><?=$orderdata['back_note'];?></td>
                                            </tr>
                                        <?}?>
                                        <? if ($orderdata['product_flow']==7 or $orderdata['product_flow']==3){?>
                                            <tr>
                                                <th><?=$this->lang['returner_bank'];//退貨人銀行名稱;?></th>
                                                <td><?=$orderdata['back_bank'];?></td>
                                            </tr>
                                            <tr>
                                                <th><?=$this->lang['returner_account'];//退貨人銀行帳戶;?></th>
                                                <td><?=$orderdata['back_account'];?></td>
                                            </tr>
                                            <tr>
                                                <th><?=$this->lang['returner_reason'];//退貨原因;?></th>
                                                <td><?=$orderdata['back_note'];?></td>
                                            </tr>
                                        <?}?>
                                    </tbody>
                        ...
                        <form id="order_detail_form" method="POST">
                            <table class="table table-h sum-table">
                                <tfoot>
                                    ...
                                    <tr>
                                        <td><?=$this->lang['o_82'];//使用購物金?></td>
                                        <td><?=$orderdata['use_shopping_money'];?></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <?=$this->lang['o_72'];//本次購買可再護得:紅利?>
                                            <span class="color01"><?=$orderdata['bonus'];?>
                                                <?=$this->lang['o_70'];//點?>
                                            </span>
                                            <? /*if(!empty($orderdata['receipt_num'])): //20160624-有發票號才顯示?>
                                                <input class="btn simple" id="invoice_print_action" type="button" style="width: 100px;font-size: 18px;" value='<?=$this->lang['print_invoice'];//發票列印?>'>
...
<script>
    ...
    $(function() {
        ...
        $("#invoice_print_action").click(function(){
            ...
            $("#order_detail_form").attr('action','/order/order_invoice_print');
            $("#order_detail_form").attr('target','invoice');
            $("#order_detail_form").submit();
            $("#order_detail_form").attr('target','');
        });
    });
   function check() {  
       var f = document.forms[0];  
