    <?=form_open($form,array('enctype'=>"multipart/form-data","id"=>"search_form"));?>
				<section class="content has-side">
					<div class="title"><?=$this->lang_menu['organization'];//組織表?></div>
					<!--
    ...
    <?php if (!empty($dbdata)): ?>
       <?php foreach ($dbdata as $key => $value): ?>
        					<thead>
							<tr>
								<th class="head-th" colspan="7"><?=$this->lang['no'];//第?> <?=$i?> <?=$this->lang['generation'];//代?></th>
							</tr>
							</thead>
							<thead>
								<tr>
    ...
    <?php endforeach; ?>
		<?php else: ?>
							<p align="center"><?=$mdata['member_num'];//經銷商編號?>(<?=$mdata['d_name'];//經銷商姓名?>)無組織資料</p>
    <?php endif; ?>
						</table>
						<?php if(!empty($s_account)){ ?>
						<a href="/gold/organization" class="btn back"><i class="icon-chevron-left" aria-hidden="true"></i> <?=$this->lang['back_list'];//回列表?></a>
						<?php } ?>
