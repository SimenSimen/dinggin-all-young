    ...
    <fieldset class="config-border">
        ...
        <table id="member_list" class="table table-bordered table-condensed">
    		...
    		<tr bgcolor='<?//=$member_auth_color[$key]?>' align='right'>
            	<td colspan="4" class='center_td white_td'>紅利折抵 <b><?=$dividend?></b> 元，總金額：<span style="color:red;font-weight:bold;"><?=number_format($dbdata['total_price'])?> 元</span></td>
            </tr>
        </table>
    </fieldset>
    <fieldset class="config-border">
...
$(function() {
	$("#return_now_action").click(function(){
		...
		$("#add_fix_form").attr('action','/bonus/sale_list/<?php echo isset($_SESSION["AT"]["where"]["by_id"])?$_SESSION["AT"]["where"]["by_id"]:"";?>');
		$("#add_fix_form").submit();
	});
});
</script>