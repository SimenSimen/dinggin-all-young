          ...
          <? foreach ($rule as $value):?>
            <tr>
                ...
                <td class='member_list_input_td'>
                  <input type="number" class="form-control" name="rule[<?=$value['d_id']?>]" value='<?=$value['d_val']?>'/>
                </td>
            </tr>
          <? endforeach;?> 
          <tr>
...
<script>
$('input[name="rule[5126]"]').on('change', () => {
  if ($('input[name="rule[5126]"]')[0].value > 50) {
    alert('%數上限為50')
    $('input[name="rule[5126]"]')[0].value = 50;
  }
})
</script>
</html>
