...
<head>
  ...
  <!-- seo -->
  <title><?=$my_e_news['name']?> - <?=$web_config['title']?></title>
  ...
  <meta property="og:title"       content="<?=$my_e_news['name']?>"></meta>
  <meta property="og:description" content="<?=$my_e_news['content']?>"></meta>
  <meta property="og:image"       content="<?=$base_url.substr($img_url, 2).$my_e_news['filename']?>"></meta>
  ...
  <div role="main" class="ui-content jqm-content" data-theme="d" id="contentMain" name="contentMain" style="text-align:center;">
    ...
    <table id='share_btn_table' style="width: 100%;">
      <tr>
          ...
          <td>
              <a href="https://line.naver.jp/R/msg/text/?<?=$my_e_news['name']?>%0D%0A<?=$actual_link?>" rel="nofollow" ><img src="<?=$base_url?>/images/share_btn/line_35x35.png" class='share' style='border:0;'/></a>
          </td>
          <td>
              <!-- Email -->
              <a href="mailto:?subject=<?=$my_e_news['name']?>&body=<?=$my_e_news['content']?><br>網址：<br><?=$actual_link?>">
    ...
    <p style="font-size: 1.3em; font-family:'微軟正黑體';"><?=$my_e_news['name']?></p>

    <img style="width:100%;" src='<?=$base_url.substr($img_url, 2).$my_e_news['filename']?>'>
    
    <p style="font-size: 1.3em; font-family:'微軟正黑體';"><?=nl2br($my_e_news['content'])?></p>
