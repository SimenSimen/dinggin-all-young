...
<div id="container">
  ...
  <div style="width: 140px; border-radius: 5px; border: 2px solid #B2A171; padding: 5px; float:left;"><p style="font-size:20px;">會員電子報列表</p></div>
  <?php if($add_push): ?>
    <a href='/member/add' target='_blank' class="aa5" style="float:right; font-size:20px; margin-left: 0px; padding: 10px;">新增會員電子報 <i class="fa fa-commenting-o"></i></a>
  <?php endif; ?>
  <br>
          ...
          <div class='content_list' id='content_list'>
            <table id="push_table">
              ...
              <?php if(!empty($page_data)): ?>
                ...
                <tr>
                  <td><?=$value['d_create_time']?></td>
                  <td id="del_<?=$value['p_id']?>"><?=$value['d_title']?></td>
                  <td><?=$value['d_status']?></td>
                  <td>
                  <a class="aa5" style="cursor: pointer; font-size: 22px;" href="/member/R/<?=$value['d_id']?>/<?=$page?>">
                    <i class="fa fa-eye" aria-hidden="true"></i>
                  </a>
                  <?php if($value['edit']): ?>
                    <a href="/member/edit/<?=$value['d_id']?>" class="aa5" style="cursor:pointer; font-size: 22px;" target='_blank'>
                      <i class="fa fa-pencil-square-o"></i>
                    </a>
                  <?php endif; ?>
                    <a class="aa5" style="cursor: pointer; font-size: 22px;" onclick="del_check(<?=$value['d_id']?>)">
