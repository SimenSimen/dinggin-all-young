...
<div id="container"><div class="_width">
   <p style="font-size:20px;"><?=$lang['BrandSort']?>
    <a href="#" class="why">?</a>
    <div class='prompt-box'>
      <p>請拖拉您要排序的品牌，然後點選「儲存」</p>
      <p>1.此處品牌，不論「上架」、「下架」均會做顯示，故會導致品牌總數量與顯示數量不相符。</p>
      <p>2.紅色字體為已下架。</p>
    </div>
  </p>
  <br>
  <!--主介面區-->
  <form action='/brands/product_brand_sort' method='post' name='form_product_hot_order' id='form_product_hot_order'>
  <p style="width: 100%; border-radius: 5px; margin-left: 4px; vertical-align: middle;">
    <input style="background: #CCC; color: black;" class="aa3" type="button" value="<?=$lang['BrandRecords']?> (<?=$total?>)">
    ...
    <div id="sortable">
      <?php foreach ($dbdata as $key => $value): ?>
        <div class="prd_box">
          ...
          <div class="prd_img"><img class="prd_imgSize" src="<?=$dbdata[$key]['image']?>"></div>
        </div>
      <?php endforeach; ?>
    </div>
  </form>
