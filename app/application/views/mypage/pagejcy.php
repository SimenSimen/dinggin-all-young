...
<? if($TotalRecord<>0) {?>
	<div class="pagination_box" id="pagination">
		...
		<div class="page-info">
			<select class="form-control" name="" id="" onchange="changepage(this.value);">
				<li>
				<?php if($TotalPage==0):?>
					<li><a href="javascript:void(0);" value="1" >1</a></li>
				<?php else:?>
					<?php //for($i=1;$i<=$TotalPage;$i++):?>
					<?php for($i=$PageToLink["pstar"];$i<=$PageToLink["pend"];$i++):?>
					<option value="<?php echo $i;?>" <?php if(($CurrectPage)==$i):?>selected<?php endif;?>><?=$this->lang['page_no'];//第?><?=$i;?><?=$this->lang['page_page'];//頁?></option>
				    <?php endfor;?>
				<?php endif;?>
				</li>
			</select>
