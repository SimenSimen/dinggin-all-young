                  ...
                  <?php foreach ($prd as $key => $value): ?>
                    ...
                    <input type="hidden" id="price<?=$key?>" value="<?=$value['prd_price00']?>">
                      ...
                      <?php if($value['prd_lock_amount'] < $value['prd_amount']): ?>
                        <input type="number" style="width: 45px; text-align: center;" min='1' max="<?=($value['prd_lock_amount']-$snum[$key])?>" id="buy<?=$key?>" name="buy[<?=$key?>]" value="<?=$prd_num[$key]?>">
                      <?php else: ?>
                        <input type="number" style="width: 45px; text-align: center;" min='1' max="<?=($value['prd_amount']-$snum[$key])?>" id="buy<?=$key?>" name="buy[<?=$key?>]" value="<?=$prd_num[$key]?>">
                      <?php endif; ?>  
                      ...
                      <td class="cart_item_total"><span id="count<?=$key?>"><?=number_format($prd_total[$key])?></span></td>
...
$(function() {
  $('#buy<?=$key?>').change(function() {
    ...
    if($_buy != '' || $_buy != '0')
    {
      if ($_buy > $_amount)
      {
        alert("<?=ExcessiveNumber?>" + $_amount);  //您輸入的數量過大，目前庫存為：
        return false;
      }
      else
      {
    ...
    else
    {
      ...
      if($_buy == Number(0))
      {
        alert("<?=QuantityNot0?>");//數量不可以是0
        value_return();
      }
      else
      {
        alert("<?=ScanQuantity?>");//請輸入數量
        value_return();
      }
    }
  });
  ...
  function value_return() {
    ...
    return false;
              ...
              <div class="buyer_div">
                ...
                <div >
                  <h3><?=$ReceiverInformation?><h4><input type="checkbox" id="somemember" value="1"><?=$somemdata;//同會員資料?></h4>&nbsp;&nbsp;
                    <?php //if ($by_id == 0): ?>
                  ...
                  <table id='buyer_table'>
                    ...
                    <tr>
                      ...
                      <td style="text-align:left;">
                        ...
                        <? endif;?>
                        <input type="text" name='receipt_address' id='receipt_address' class="required" maxlength='255'>
                        <input type="radio" name="someadd" id='same_address'><?=$somesend;//同收貨地址?>
                        <input type="radio" name="someadd" id='member_address'><?=$somemadd;//同會員地址?>
                      </td>
                    </tr>
                    <?php endif; ?>
                    <tr>
