...
<html lang="en" class="no-js">
  <head>
      ...
      <link href="https://ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/themes/flick/jquery-ui.min.css" rel="stylesheet">
    ...
    <header>
     <div class="www">
      <div class="row ">
           ...
           <div class="col-xs-8"><h1 style="font-size:16px;"><?=$prd['prd_name']?></h1></div>
           <div class="col-xs-2 text-right">
          
                <a id="cart_link" class="cart" href="javascript:void(0);" data-placement="bottom" data-trigger="hover"  
                data-content="<?=$this->lang['ordering'];//已選購商品?><br><span style='color:#DA5049'>0</span> <?=$this->lang['pic'];//件?><br><?=$this->lang['total'];//總金額(TWD)?><br><span style='color:#DA5049'>0</span> <?=$this->lang['pri'];//元?>" >
    ...
    <div class="detail-L">
       ...
       <div class="detail-R"> 
           <div class="row">
               ...
               <div class="col-lg-2 col-md-3 col-xs-4"><?=$this->lang['sale'];//建議售價?></div>
               <?php if ($prd['prd_price01'] != ''): ?>
               <div class="col-lg-10  col-md-9  col-xs-8">$ <?=number_format($prd['prd_price01'])?></div>
               <?php else: ?>
               <div class="col-lg-10  col-md-9  col-xs-8"><?=$this->lang['no'];//無?></div>
               <?php endif; ?>

               <div class="col-lg-2 col-md-3  col-xs-4"><?=$this->lang['offer'];//特價?></div>
               <div class="col-lg-10 col-md-9 col-xs-8"><mark>$ <?=number_format($prd['prd_price00'])?></mark></div>
               <? if(!empty($prd['d_mprice'])):?>
                 <div class="col-lg-2 col-md-3  col-xs-4"><?=$this->lang['empprice'];//員工價?></div>
                 <div class="col-lg-10 col-md-9 col-xs-8"><mark>$ <?=number_format($prd['d_mprice'])?></mark></div>
               <? endif;?>
               <?php if ($prd['prd_active'] == 0): ?>
                <div class="col-lg-2 col-md-3  col-xs-4"><?=$this->lang['stoca'];//庫存數量?></div>
                <div class="col-lg-10  col-md-9 col-xs-8"><? if ($prd['prd_amount'] > 0) echo '充足'; else echo '缺貨'; ?>   &nbsp;
                <a id="prd_amount" href="#prd_amount_div"><?=$this->lang['only'];//僅供參考?></a>
                  <div id="prd_amount_div" style="display: none;"><?=$this->lang['sort'];//實際商品數量以訂購銷存順序為準?></div>
                  </div>
               <?php endif; ?>
               ...
               <div class="col-lg-2 col-md-3  col-xs-4"><?=$this->lang['pay'];//付款方式?></div>
                ...
                <?php endif;?>
               <div class="col-lg-2 col-md-3  col-xs-4"><?=$this->lang['logis'];//交貨方式?></div>
                ...
                <?php endif; ?>
                <?php if(!empty($fee)):?>
                  <div class="col-lg-10 col-md-9  col-xs-8" style="float: right;"><?=$this->lang['shopall'];//購物滿?> <span style="color:red;"><?=number_format($fee)?></span> <?=$this->lang['pri'];//元，?><span style="color:red;"><?=$this->lang['nopay'];//免運費?></span></div>
                <?php endif; ?>
                <?php if($prd['restrice_num']!='0'):?>
                  <div class="col-lg-10 col-md-9  col-xs-8" style="float: right;"><?=$this->lang['onelim'];//每人限購?> <span style="color:red;"><?=number_format($prd['restrice_num'])?></span><?=$this->lang['par'];//組?></div>
                <?php endif; ?>
      </div> 
      ...
      <div class="row">
               ...
               <div class="col-lg-4 col-md-3 col-sm-4 col-xs-5" >
                <!--<input type="number" name="amount" id="prd_num" min="1" max="4" value="1" class="form-control" style="width:120px" >-->
                  ...
                  <?php if($prd['prd_lock_amount'] < $prd['prd_amount']):?>
                    <?php for($i = 1; $i < ($prd['prd_lock_amount']+1)-$snum; $i++):?>
                      <option value="<?php echo $i?>"><?php echo $i?></option>
                    <?php endfor; ?>
                  <?php else: ?>
                    <?php for($i = 1; $i < ($prd['prd_amount']+1)-$snum; $i++):?>
                      <option value="<?php echo $i?>"><?php echo $i?></option>
                    <?php endfor; ?>
                  <?php endif; ?>           
                  </select>
               ...
               <div class="col-lg-8 col-md-9 col-sm-8 col-xs-7">
                <button class="aa4" id="add_to_cart"><?=$this->lang['cart'];//放入購物車?><span class="icon-cart32"></span></button>
             ...
             <? else:?>
              <div class="col-lg-8 col-md-9 col-sm-8 col-xs-7">
                <button class="aa4" ><?=$this->lang['limit'];//已達限購數量?><span class="icon-cart32"></span></button>
              </div>
             <? endif;?>
            </div> 
         </section>  
       ...
       <div class="tabs">
                <ul class="tab-links">
                    <li class="active"><a href="#buy1" ><?=$this->lang['info'];//商品介紹?></a></li>
                    <li><a href="#buy2"><?=$this->lang['spec'];//商品規格?></a></li>
                    <li><a href="#buy3"><?=$this->lang['butinfo'];//購買說明?></a></li>
                    <li><a href="#buy4"><?=$this->lang['shipp'];//運送規則?></a></li>
                </ul>
...
<!--2商品規格-->
                  <div id="buy2" class="tab">
                    <?php if (!empty($prd_specification_content)): ?>
                      ...
                      <?php endforeach; ?>
                      <?php else: ?>
                        <p><?=$this->lang['noinfo'];//目前沒有任何關於商品規格的資訊?></p>
                    <?php endif; ?>
<!-- 保固範圍-->
                    <?php if ($prd['prd_assurance_range'] != '' || $prd['prd_assurance_date'] != ''): ?>                    
                      
                      <?php if ($prd['prd_assurance_range'] != ''): ?>
                        <p><?=$this->lang['warra'];//保固範圍?>：<?=$prd['prd_assurance_range']?></p><br/>
                      <?php endif; ?>
                      
                      <?php if ($prd['prd_assurance_date'] != ''): ?>
                        <p><?=$this->lang['warrper'];//保固期限?>：<?=$prd['prd_assurance_date']?></p><br/>
                      <?php endif; ?>
...
<!--3購買說明-->
                  ...
                  <p class="prd_content"><strong>【<?=$this->lang['shopinfo'];//購物說明?>】</strong><br><br>
                    <?php if ($store['cset_paid_'.$this->setlang] != ''): ?>
                      <p><?=$store['cset_paid_'.$this->setlang]?></p>
                      <?php else: ?> 
                      <p><?=$this->lang['no'];//無?></p>
                    <?php endif; ?>
                  </div>
<!--4運送規則-->
                  <div id="buy4" class="tab">
                    <?php if ($store['cset_ship_'.$this->setlang] != ''): ?>
                      <p><?=$store['cset_ship_'.$this->setlang]?></p>
                        <?php else: ?> 
                        <p><?=$this->lang['no'];//無?></p>
                    <?php endif; ?>
  ...
  <?php if($store['cset_share_btn']): ?>
    <section class="www" id="share"> 
      <h3><?=$this->lang['share'];//分享此商品?></h3>
      <a class="shareTo fb" href="javascript: void(window.open('https://www.facebook.com/share.php?u='.concat(encodeURIComponent(location.href)) ));" title="facebook"><span class="icon-facebook2"></span></a>
      <a class="shareTo twitter" href="javascript: void(window.open('https://twitter.com/home/?status='.concat(encodeURIComponent(document.title)) .concat(' ') .concat(encodeURIComponent(location.href))));" title="twitter"><span class="icon-twitter"></span></a>
      <a class="shareTo line" href="https://line.naver.jp/R/msg/text/?<?=$prd['prd_name']?>-<?=$web_config['title']?>%0D%0A<?=$prd_url?>" title="line"></a>  
      ...
      <a class="shareTo sina" href="javascript:(function(){window.open('https://v.t.sina.com.cn/share/share.php?title='+encodeURIComponent(document.title)+'&url='+encodeURIComponent(location.href)+'&source=bookmark','_blank','width=450,height=400');})()"  title="新浪微博" ><span class="icon-weibo"></span></a>
      <!-- <a class="shareTo weChat" href="#"  title="WeChat 微信"><span class="icon-wechat"></span></a> -->
      <a class="shareTo plurk" href="javascript: void(window.open('https://www.plurk.com/?qualifier=shares&status=' .concat(encodeURIComponent(location.href)) .concat(' ') .concat('&#40;') .concat(encodeURIComponent(document.title)) .concat('&#41;')));" title="Plurk 噗浪"></a>
      ...
      <a class="shareTo qq" href="javascript: void(window.open('https://share.v.t.qq.com/index.php?c=share&a=index&url='.concat(encodeURIComponent(location.href)) +'&title=' .concat(encodeURIComponent(document.title)) ));"  title="腾訊微博"><span class="icon-tencent-weibo"></span></a>
      <a class="shareTo ren" href="javascript: void(window.open('https://widget.renren.com/dialog/share?resourceUrl='.concat(encodeURIComponent(location.href)) +'&title=' .concat(encodeURIComponent(document.title)) ));"  title="人人"><span class="icon-renren2"></span></a>   
      <a class="shareTo qzone" href="javascript: void(window.open('https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url='.concat(encodeURIComponent(location.href))+'&title='.concat(encodeURIComponent(document.title)) ));"  title=""></a>       
    </section>
  <?php endif; ?>
  
      <section class="www" id="relativeP"> 
      <h3><?=$this->lang['someinfo'];//相關產品?></h3>
