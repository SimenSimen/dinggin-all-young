...
<head>
  ...
  <style type="text/css">
    * { font-family: 'Microsoft Jhenghei'; }
    ...
    .prd_cname { overflow: hidden; text-overflow: ellipsis; white-space: pre-line; width: 98%; }
    .aa3{
...
<body>
  ...
  <div id="container" style="min-height:300px">
    ...
    <header>
      ...
      <?php if (!empty($prdc)): ?>
        <?=$store['cset_name']?>
        <a style="display: inline-block;" href="<?=$iqr_url?>" ><img style="width: 30px; top: -3px; position: relative;" src="/images/cart/cart_home.png" title="<?=$this->lang['backindex'];//回首頁?>"></a>
        <a style="display: inline-block;" href="/cart/search_engine/<?=$store['cset_code']?>" ><img style="width: 30px; top: -3px; position: relative;" src="/images/cart/cart_search.png" title="<?=$this->lang['strseartch'];//查詢?>"></a>
        <a id="cart_link" title="<?=$this->lang['strorder'];//購物明細?>" style="display: inline-block;" href="/cart/check/<?=$cset_code?>/1" data-content="<span style='font-size:1.7em;'>購物車內有<span style='color:red;'>0</span>件商品</span><br><span style='font-size:1.7em;'>共<span style='color:red;'>0</span>元(TWD)</span>" data-placement="bottom" data-trigger="hover"><img style="width:30px; top: -3px;position: relative;" src="/images/cart/cart.png"></a>  
        <?php if ($user_login): ?>
          <a style="display: inline-block;" href="/cart/record/<?=$cset_code?>" ><img style="width: 30px; top: -3px;position: relative;" src="/images/cart/cart_record.png" title="<?=$this->lang['strseart'];//訂單查詢?>"></a>
    ...
    <?php
    $urlGo = '/cart/store/'.$store['cset_code'];
    if (!empty($prdc)): ?>
      <ol id="filters" style="margin: 0px auto;padding: 0 0 0 10px;">
        <a href="<?=$urlGo?>/a"><li data-filter="all" <?if ($product_cid == 'a') echo 'class="active"';?>>All</li></a>
        <?php foreach ($prdc as $key => $value): ?>
          <a href="<?=$urlGo?>/<?=$value['prd_cid']?>">
          <li data-filter="filter_<?=$key?>" <?if ($product_cid == $value['prd_cid']) echo 'class="active"';?> title='<?=$value['prd_cname']?>'><div class='prd_cname'><?=$value['prd_cname']?></div></li>
          </a>
        <?php endforeach; ?>
      </ol>
    <?php endif; ?>
              ...
              <?php if (!empty($prd[$value['prd_cid']])): ?>
                <?php foreach ($prd[$value['prd_cid']] as $sub_key => $sub_value): ?>
                  <li class='prd_info' id='prd_info_<?=$sub_value['prd_id']?>' data-filter-class='["filter_<?=$key?>", "products"]' title='<?=$sub_value['prd_name']?>'>
                    ...
                    <STRIKE>$<?=number_format($sub_value['prd_price01'])?></STRIKE>
                        $<?=number_format($sub_value['prd_price00'])?></p>
                  </li>
                <?php endforeach; ?>
              <?php endif; ?>
            <?php endforeach; ?>
  ...
  <footer>
   ...
   <?php endif;?>
    ...
    <?php endif; ?>
     ...
     <div class="clear"></div>
     <div>
        <a class="fb" href="javascript: void(window.open('https://www.facebook.com/share.php?u='.concat(encodeURIComponent(location.href)) ));" ><img src="/images/fb.png"></a>
        <a class="twitter" href="javascript: void(window.open('https://twitter.com/home/?status='.concat(encodeURIComponent(document.title)) .concat(' ') .concat(encodeURIComponent(location.href))));" ><img src="/images/twitter.png"></a>
        ...
        &nbsp;&nbsp;
        <a href="javascript: void(window.open('https://line.naver.jp/R/msg/text/?'.concat(encodeURIComponent(document.title)) .concat(' ') .concat(encodeURIComponent(location.href))));" ><img src="/images/share_btn/line_35x35.png" style="width:40px;height:40px;" /></a>
