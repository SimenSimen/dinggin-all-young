...
<head>
  ...
  <!-- seo -->
  <title><?=$FriendTicketQuickLink?> <?=$web_config['title']?></title>
  ...
  <!--jQuery-->
  <script type="text/javascript" src="https://code.jquery.com/jquery-1.10.2.min.js"></script><!--library-->
...
//		$outurlstr=$base_url.'form/index8/'.$ecp_id.'/'.$mid.'/'.$account.'/'.$viewtype;
    ...
    $(function(){
      $('#text').click(function(){
        window.open('<?=$outurlstr?>', '<?=$FriendTicket?>', config='scrollbars=1,height=616,width=420,left=870,resizable=yes,scrollbar=yes');
      });
    });
  </script>
...
<div data-role="header">
  <h1><?=$FriendTicketQRcode?></h1>
</div>

<div data-role="content" data-theme='c'>

<span style="font-family:'微軟正黑體';font-size:1.05em;"><?=$UsePhoneScan?><br><?=$QuickFriendTicket?><br><?=$ClickPictureDownload?><br></span>
...
<br>
<!-- qrcode url -->
<textarea readonly="true" style="resize:none;width:85%;cursor: pointer;height:62.8px;" name="iqr_url" id="text"><?=$ClickOpen?> <?=$outurlstr?></textarea>
