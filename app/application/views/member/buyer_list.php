    ...
    <?php if (!empty($dbdata)): ?>
        ...
        <?php foreach ($dbdata as $key => $value): ?>
          ...
          <tr bgcolor='<?=$member_auth_color[$key]?>'>
            ...
            <td class='center_td white_td'><?=$value['d_dividend']?></td>
            <td class='center_td white_td'><?=$value['d_shopping_money']?></td>
            <td class='center_td white_td'><?=$value['Pname']?></td>
            <td class='center_td white_td'><?=$value['upline']?></td>

<!--
            <td class='center_td white_td'> 
            			<canvas id="qrcodeCanvas<?=$icount;?>" width="240" height="240" style="display: none;"></canvas>
            			<script>
                			jQuery('#qrcodeCanvas<?=$icount;?>').qrcode({ width : 240, height : 240, text : "https://<?=$_SERVER['HTTP_HOST'].'/gold/register/'.base64_encode($value['d_account']);?>"}); 
  							var canvas<?=$icount;?> = document.getElementById('qrcodeCanvas<?=$icount;?>'),context = canvas.getContext('2d');
            			</script>
            			<br>
            			<a href="#" download="<?=$value['name']?>.png" onclick="this.href=canvas<?=$icount;?>.toDataURL();" >下載</a>
            </td>
-->
            <td class='center_td white_td'> 
