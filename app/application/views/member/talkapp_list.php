...
<body background="<?=$web_config['admin_background_image']?>" bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="text-align: center">
  <table >
    <tr>
      <td>
        <input type="text" name="s_name" value="<?=$s_name?>" placeholder="輸入查詢帳號">
        <select name="s_type">
          <option value="0" <?=($s_type=='0')?'selected':'';?>>一般會員</option>
          <option value="1" <?=($s_type=='1')?'selected':'';?>>經營會員</option>
        </select>
        ...
        <?php foreach ($dbdata as $key => $value): ?>
          <tr bgcolor='<?=$member_auth_color[$key]?>'>
            ...
            <td class='center_td white_td'> 
              <a href="javascript:void(0);" onclick="top.frames['content-frame'].location='/member/ebhmlist/<?=$value['m_id']?>'">
