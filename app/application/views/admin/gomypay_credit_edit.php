...
<head>
  ...
  <!-- seo -->
  <title>萬事達線上刷卡(Gomypay Credit)金流設定 - <?=$web_config['title']?></title>
...
<body scroll="yes" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="text-align: center; overflow-x: hidden;overflow: -moz-scrollbars-vertical;">
  ...
  <div class='uform'>
    ...
    <!--新增商品分類form-->
    <form action='/admin/gomypay_credit_edit/<?=$member['member_id']?>/1' method='post' id='qrcode_edit'>
      <h3 style="font-family: '微軟正黑體';">萬事達線上刷卡(Gomypay Credit) - 金流設定</h3>
        <table class='table product_add_table' id='product_add_table' style="width:90%;">
          ...
          <tr>
            ...
            <td class='table-cell-left'><input value="<?=$iqrt['business_account']?>" type='text' class='form-control' id="business_account" name='business_account' placeholder='萬事達商店代號'></td>
          </tr>

          <tr>
            <td class='table-cell-right'>交易驗證密碼</td>
            <td class='table-cell-left'><input value="<?=$iqrt['business_hashiv']?>" type='text' class='form-control' id="hashiv" name='business_hashiv' placeholder='交易驗證密碼'></td>
          </tr>
