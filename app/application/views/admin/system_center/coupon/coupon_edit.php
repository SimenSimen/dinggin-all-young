...
<body scroll="yes" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="text-align: center; overflow-x: hidden;overflow: -moz-scrollbars-vertical;">
  ...
  <div class='uform'>
    ...
    <!--新增好友分享券form-->
    <form action='/coupon/ecoupon/edit/0/<?=$edit_ecp['ecp_id']?>' method='post' name='form_ecoupon_edit' id='form_ecoupon_edit' enctype="multipart/form-data" onsubmit="return chk_val()">
      <h3 style="font-family: '微軟正黑體';"><?=$TellFriendVoucher?></h3>
        <table class='table ecoupon_edit_table' id='ecoupon_edit_table' style="width:90%;">
          ...
          <tr>
            ...
            <td class='table-cell-left'><input type='text' value="<?=$edit_ecp['name']?>" class='form-control required' name='name' id='name' maxlength="32" placeholder='<?=$Must32Char?>'></td>
          </tr>
              ...
              <?php if(!empty($category)): ?>
                <select name="category_id" class="required">
                  <?php foreach ($category as $key => $value): ?>
                    <option value="<?=$value['category_id']?>" <?php if($edit_ecp['category_id'] == $value['category_id']):?>selected<?php endif;?>><?=$value['c_name']?></option>
                  <?php endforeach; ?>
                </select>
              <?php endif; ?>
            </td>
		  ...
		  <tr>
			<td class='table-cell-right'><?=$status?></td>
			<td class='table-cell-left'>
				<select name="coup_enable">
					<option value='1' <?=($edit_ecp['enable'] == 1)?'selected':'';?>>公開中</option>
					<option value='0' <?=($edit_ecp['enable'] == 0)?'selected':'';?>>隱藏中</option>
				</select>
			</td>
	      </tr>
          <tr>
