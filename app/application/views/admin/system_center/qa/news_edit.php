...
<body scroll="yes" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="text-align: center; overflow-x: hidden;overflow: -moz-scrollbars-vertical;">
  ...
  <div class='uform'>
    ...
    <!--新增好友分享券form-->
    <form action='/<?=$this->mode_function;?>/enews/edit/0/<?=$edit_enews['qaa_id']?>' method='post' name='form_enews_edit' id='form_enews_edit' enctype="multipart/form-data" onsubmit="return chk_val()">
      <h3 style="font-family: '微軟正黑體';">編輯訊息</h3>
        <table class='table enews_edit_table' id='enews_edit_table' style="width:90%;">
          ...
          <tr>
            ...
            <td class='table-cell-left'><input type='text' value="<?=$edit_enews['qaa_title']?>" class='form-control required' name='name' id='name' maxlength="255"></td>
          </tr>
              ...
              <?php if(!empty($category)): ?>
                <select name="category_id" class="required">
                  <?php foreach ($category as $key => $value): ?>
                    <option value="<?=$value['qaag_id']?>" <?php if($edit_enews['qaag_id'] == $value['qaag_id']):?>selected<?php endif;?>><?=$value['qaag_name']?></option>
                  <?php endforeach; ?>
                </select>
              <?php endif; ?>
            </td>
          ...
          <tr>
            ...
            <td class='table-cell-left'>
              <textarea name='content' class='form-control' id='ckeditor'><?=$edit_enews['qaa_content']?></textarea>
            </td>
          </tr>
		  <tr>
