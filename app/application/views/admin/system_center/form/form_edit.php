  ...
  <div class='uform'>
    ...
    <h5 style="font-family: '微軟正黑體';">※&nbsp;<?=$FillActivityEx?></h5>
      <table class='table' id='uform_table'>
        ...
        <tr>
          <td class='table-cell-right'>欄位名稱</td>
          <td> </td>
        </tr>
        <?
        $icheck=false;
        $ufm_col_name=explode("*#",$uform['ufm_col_name']);
  		foreach ($ufm_col_name as $key => $value) {
  			if($icheck){
      	?>
        <tr>
          <td colspan="2" ><input name="ufm_col_name[]" id="ufm_col_name[]" value="<?=$value?>"></td>
        </tr>
        	<?
        	}
        	$icheck=true;
		...
		<tr>
			...
			<td class='table-cell-left'>
				<select name="ufm_enable">
					<option value='1' <?=($uform['enable'] == 1)?'selected':'';?>>公開中</option>
					<option value='0' <?=($uform['enable'] == 0)?'selected':'';?>>隱藏中</option>
				</select>
			</td>
		</tr>
        <tr>
