	...
	<script>
		$(function() {
			...
			$('.btn-status').click(function() {
				$('#list_form').attr('action', '/eform/article_status');
				$('#list_form').submit();
			});
			...
			<table id='list' class='table table-hover table-bordered table-condensed' style="width:80%;">
				...
				<?php foreach ($form as $key => $value): ?>
				    <tr>
						...
						<td><?=$value['enable']?></td>
				    	<td>
				    		<a onclick="window.open('/eform/uform_sign_up_show/sign_up_<?=$value['ufm_id']?>/<?=$value['member_id']?>', '<?=addslashes($value['ufm_name'])?>', config='height=500,width=700');"><i class="fa fa-file-text-o" aria-hidden="true"></i></a>
				    		<a href="/eform/uform_edit/<?=$value['ufm_id']?>"><i class="fa fa-pencil-square-o"></i></a>
				    		<a onclick="window.open('/form/index/<?=$value['ufm_id']?>/<?=$value['member_id']?>', '<?=addslashes($value['ufm_name'])?>', config='height=500,width=700');"><i class="fa fa-qrcode"></i></a>
			    		</td>
				    </tr>
				<?php endforeach; ?>
				</tbody>
