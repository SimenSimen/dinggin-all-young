      ...
      <p style="font-family: '微軟正黑體'; font-size: 15px;">(若不輸入連結，則保有初始的載點)</p>
        <table class='table product_add_table' id='product_add_table' style="width:90%;">
          ...
          <tr>
            ...
            <td class='table-cell-left'><input value="<?=$iqr['apk_release']?>" type='text' class='form-control' id="apk_release" name='apk_release' placeholder='Ex: https://www.gooogle.com'></td>
          </tr>

          <tr>
            <td class='table-cell-right'>iOS_Qrcode</td>
            <td class='table-cell-left'><input value="<?=$iqr['ipa_release']?>" type='text' class='form-control' id="ipa_release" name='ipa_release' placeholder='Ex: https://www.gooogle.com'></td>
          </tr>
          <tr>
...
<script>
  $(function(){
    ...
    $('#qrcode_btn').click(function(){
      ...
      {
        if($('#apk_release').val().indexOf('https://') != 0 && $('#apk_release').val().indexOf('https://') != 0 && $('#apk_release').val() != '')
        {
          alert("Android Qrcode 網址須加上「https://」或「https://」");
          $('#apk_release').focus();
          return false;
        }
        else if($('#ipa_release').val().indexOf('https://') != 0 && $('#ipa_release').val().indexOf('https://') != 0 && $('#ipa_release').val() != '')
        {
          alert("iOS Qrcode 網址須加上「https://」或「https://」");
          $('#ipa_release').focus();
          return false;
        }
        else
