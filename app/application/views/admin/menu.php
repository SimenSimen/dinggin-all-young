...
<div id='menu-link-table'>
  ...
  <select style="float: right;" id="setlang">
        ...
        <option value="IND" <?=($this->setlang=='IND')?'selected':'';?>>印尼</option>
    </select>
  <!--推播管理-->
  <p><button class="btn btn-primary" type="button" onclick="top.frames['content-frame'].location='/push/index'">推播訊息</button></p>
  

  <?php if($_SESSION['AT']['account_name'] == 'super' || $_SESSION['AT']['account_name'] == 'admin' ): ?>
    <!--會員資料、新增會員、期限管理-->
  ...
  <?php if($_SESSION['AT']['account_name'] == 'super'): ?>
    <!--版型設定-->
    <p><button class="btn btn-warning" type="button" onclick="top.frames['content-frame'].location='/admin/style_config'">版型設定</button></p>
    <p><button class="btn btn-info" type="button" onclick="top.frames['content-frame'].location='/admin/domain_edit'">網域設定</button></p>
  <?php endif; ?>
  ...
  <p><button class="btn btn-success" type="button" onclick="top.frames['content-frame'].location='/shop/list_page'">門市管理</button></p>
