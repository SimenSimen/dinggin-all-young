...
<head>
  ...
  <!-- seo -->
  <title>新增推播訊息 - <?=$web_config['title']?></title>
...
<body scroll="yes" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="text-align: center; overflow-x: hidden;overflow: -moz-scrollbars-vertical;">
  <div>
    ...
    <form action='/push/add' method='post' name='form_push_msg_add' id='form_push_msg_add' enctype="multipart/form-data">
      <h3 style="font-family: 'Microsoft JhengHei';">發送一則推播訊息</h3>
        <table class='table push_msg_add_table' id='push_msg_add_table' style="width:90%;">

          <?php if ($group_push): ?>
            <tr>
              <td class='table-cell-right'>對象</td>
              <td class='table-cell-left'>群體<input type="hidden" name="broadcasting" id="broadcasting_0" value="0">
              <?php if (0): ?>
                <?php if($show_device): ?>
                  <input type="radio" name="broadcasting" id="broadcasting_0" value="0" checked>
                  <label style="font-family: 微軟正黑體; font-weight: normal;" for='broadcasting_0'>群體</label> &nbsp;&nbsp;
                  <input type="radio" name="broadcasting" id="broadcasting_1" value="1">
                ...
                <td class="table-cell-left">
                  ...
                  <select id='multiselect-options' multiple='multiple' name="subordination[]" required>
                    <?php foreach ($device_users as $key => $value): ?>
                      <option value="<?=$value['member_id']?>"><?=$value['account']?> (<?=$value['name']?>)</option>
                    <?php endforeach; ?>
                  </select>
          ...
          <tr>
            ...
            <td class='table-cell-left'><input type='text' class='form-control required' name='title' id='title' maxlength="30" placeholder='標題文字 (30)'></td>
          </tr>
          
          <tr>
            <td class='table-cell-right'>圖片</td>
            <td class='table-cell-left'><input id='image' style="font-size: 14px;" type='file' name='image'></td>
          </tr>

          <tr>
            <td class='table-cell-right'><span class='red_star'>*</span>訊息</td>
            <td class='table-cell-left'>
              <textarea style="resize: none;" name='message' class='form-control required' id='message' rows='4' maxlength='128' placeholder='將不會顯示於IPhone、IPad上'></textarea>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td class='table-cell-left'>
              <textarea id="messagebox" name="messagebox" class='form-control required' id='messagebox'></textarea>
            </td>
          </tr>
...
<script src="/js/push_add.js"></script>
<script>
  $(function (){

    if($("input[name=broadcasting]").val() == 1)
      $("#multiselect").hide();
