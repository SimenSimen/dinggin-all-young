...
class Bonus extends MY_Controller
{
    ...
    public function dividend(){        
        ...
        if($_SESSION['MT']['is_login']==1){
            ...
            $data['path_title']     ='<li><a href="/gold/member"><span>'.$this->lang['member'].'</span></a></li>'.
                                '<li><a href="/'.$this->DataName.'/dividend"><span>'.$this->lang['didetail'].'</span></a></li>';
    ...
    public function bonus_fix_list(){
        ...
        $qpage=$this->useful->SetPage('bonus','',$page_limit);        
        $data['page']=$this->useful->get_page($qpage);
        //分頁程式 end  
        $dbdata=$this->bmodel->get_bonus_data($_SESSION['d_year'],$_SESSION['d_month'],'',$Topage,$page_limit);
        foreach ($dbdata as $key => $value) {
            $dbdata[$key]['d_bonus']=number_format($value['d_bonus']);
        ...
        if($type=='detail'){    
            ...
            if(!empty($dbdata)){
                foreach ($dbdata as $value) {
                    ...
                    $data_array[]=array($value['OID'],$mnum['member_num'],$value['sName'],$value['d_type'],$value['d_pv'],$value['d_bonus'],$value['d_kv'],$d_content,$d_block,$d_bcontent,$value['d_year'],$value['d_month']);
                }        
            }else
    ...
    public function bonus_total_try(){
        ...
        foreach ($down as $key => $value) {
            $d_content=str_replace (" ","\r\n",trim($value['d_content']));
            $data_array[]=array($value['OID'],$value['member_num'],$value['sName'],$value['d_type'],round($value['d_pv'],2),round($value['d_bonus']),$value['d_kv'],$d_content,$_POST['year'],substr('00'.$_POST['month'],-2));
        }
