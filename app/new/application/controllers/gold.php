...
{	
	...
	public function home($id='',$app=''){
		...
		if(!empty($_SESSION['LA']))
			$data['lang']=$_SESSION['LA']['lang'];

		$_SESSION['AT']['account']=$id;
		$member_data=$this->mmodel->get_member_sign('','','',$id);
		$_SESSION['AT']['member_id']=$member_data['member_id'];
		$data['aid']=$_SESSION['AT']['account'];

		$display='style="display: none;"';
		//活動與報名
		$udata=$this->mymodel->SqlTotal('uform',array('lang_type'=>$this->setlang));
		$data['udata']=($udata!=0)?'':$display;
		//好友分享券
		$fdata=$this->mymodel->SqlTotal('ecoupon',array('lang_type'=>$this->setlang));
		$data['fdata']=($fdata!=0)?'':$display;
		//訊息中心 20170320 改為顯示 裡面無資料不顯示
		// $cdata=$this->mmodel->get_ck_num(1);
	
		// $isdis=($cdata!=0)?'':$display;
		// if(!empty($_SESSION['MT'])){
		// 	$cdata=$this->mmodel->get_ck_num(2);
		// 	$isdis=($cdata!=0)?'':$display;
		// }	
		// $data['cdata']=	$isdis;
		//影音專區
		$mdata=$this->mymodel->SqlTotal('ckeditor',array('type'=>4,'enable'=>1,'lang_type'=>$this->setlang));
		
		$data['mdata']=($mdata!=0)?'':$display;
		
		//公司相簿
		$adata=$this->mymodel->SqlTotal('photo_category',array('lang_type'=>$this->setlang));
		$data['adata']=($adata!=0)?'':$display;

		//附件下載 
		$rdata=$this->mymodel->SqlTotal('archive',array('lang_type'=>$this->setlang));
		$data['rdata']=($rdata!=0)?'':$display;

		//友善連結
		$ldata=$this->mymodel->SqlTotal('ckeditor',array('type'=>5,'enable'=>1,'lang_type'=>$this->setlang));
		$data['ldata']=($ldata!=0)?'':$display;
		//語言抓取
		$data['lang_list']=$lang_list=$this->mymodel->select_page_form('language_type','','d_title,d_code');
		foreach ($lang_list as $key => $value) {
			if($value['d_code']==$this->setlang)
				$NowLang=$value['d_title'];
		}
		$data['NowLang']=$NowLang;

		//view
		$this->load->view('gold/header');
		$this->load->view('gold/index', $data);
		$this->load->view('gold/footer', $data);
	}
	...
	public function login($url=''){
		...
		$this->DataName='login';
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/'.$this->DataName.'/index"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
...
//		$data['burl']=$url;
	...
	public function logout(){
		...
		$this->useful->AlertPage('/gold/login','登出成功');
		return '';
	}
	...
	public function register($upline=''){
		...
		//view
		$this->load->view('index/header', $data);
		$this->load->view('index/member/member_register', $data);
		$this->load->view('index/footer', $data);
	}
	...
	public function invite_share_ok(){
		...
		if($s_data['by_id']=="")
			$this->useful->AlertPage('/gold/invite_share',"上線邀請碼帳號不存在,請重新輸入");		//帳號不存在
		if($s_data['by_id']!=$_SESSION['MT']['by_id']){
			//登入者紅利點數加100
            ...
            $this->mymodel->insert_into('dividend',$didata);
			$this->useful->AlertPage('/gold/invite_share',"加入上線邀請碼完成");		//修改完成
		}
		else
			$this->useful->AlertPage('/gold/invite_share',"不能設定上線邀請碼為自己帳號");		//不能設定上線為自己
	}
	...
	public function member(){
		...
		//語言包
		$this->lang=$this->lmodel->config('5',$this->setlang);
		
		//view
		$this->load->view('index/header', $data);
		$this->load->view('index/member/member_nav', $data);
		$this->load->view('index/member/member', $data);
		$this->load->view('index/footer', $data);
	}
	...
	public function member_dividend_fun(){
		...
		if($_SESSION['MT']['d_is_member']==1)
			$data['shopping_money']=$this->mymodel->get_shopping_money_data($_SESSION['MT']['member_id']);
	...
	public function link($type = 'C', $element = '', $category_id = 0,$account=''){
		...
		if(!empty($data['list'])){
			...
			$data['list_detail']=$this->mymodel->get_link_data($category_id);
	...
	public function media($type = 'C', $element = '',$category_id = 0,$account = ''){
		...
		$this->DataName='movie';
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/gold/media/C/video"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
		//初始設定
		// $this->useful->iconfig($_SESSION['AT']['account']);
	...
	public function archive($type = 'C', $element = '',$category_id = 0,$account = ''){
		...
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/gold/archive/C/annex"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
		$data['banner']=$this->data['banner'];
	...
	//活動花絮
	public function photo($type = 'C', $element = '',$category_id = 0,$account = ''){
		//if(empty($_SESSION['MT'])){
		//	$this->useful->AlertPage('/gold/login','請先登入或註冊');
		//}
		$data = $this -> mod_views -> category_list($element);
		...
		$this->DataName='album';
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/gold/photo/C/photo"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
		$data['banner']=$this->data['banner'];
	...
	//活動花絮detail
	public function photo_detail($element, $category_id, $id = '',$isshareurl='',$docallapp='',$account = ''){
		//if(empty($_SESSION['MT'])){
		//	$this->useful->AlertPage('/gold/login','請先登入或註冊');
		//}
		...
		if($id!=0){
			//抓取第2層相片
			$data['list_photo']=$this->mymodel->get_photo_data($id);
			
			//抓取第1層相簿名稱
			$photodata=$this->mymodel->get_photo_one_data($id);
			$data['photo']=$photodata;
			$data['path_title']=$data['path_title'].'<li><a href="/gold/photo_detail/C/photo/'.$id.'"><span>'.$photodata['d_name'].'</span></a></li>';
		}
	...
	//最新消息
	public function news($type = 'C', $element = '',$category_id = 0,$account = ''){
		//if(empty($_SESSION['MT'])){
		//	$this->useful->AlertPage('/gold/login','請先登入或註冊');
		//}
		//$iqr = $this -> mod_views -> inner_join('member', 'iqr', array('iqr.theme_id', 'iqr.l_name', 'iqr.f_name'), array('member.account' => $account), 'member_id', 'row');
		...
		$this->DataName='news';
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/gold/'.$this->DataName.'/C/news"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
		$data['banner']=$this->data['banner'];
		...
		if(!empty($data['list'])){
			...
			$data['path_title']=$data['path_title'].'<li><a href="/gold/'.$this->DataName.'/C/news/'.$category_id.'"><span>'.$category_name.'</span></a></li>';
		}
	...
	//最新消息detail
	public function news_detail($element, $category_id, $id = '',$isshareurl='',$docallapp='',$account = ''){
		//if(empty($_SESSION['MT'])){
		//	$this->useful->AlertPage('/gold/login','請先登入或註冊');
		//}
		//$iqr = $this -> mod_views -> inner_join('member', 'iqr', array('iqr.theme_id', 'iqr.l_name', 'iqr.f_name'), array('member.account' => $account), 'member_id', 'row');
		...
		$data['path_title']=$data['path_title'].'<li><a href="/gold/'.$this->DataName.'/C/news/'.$category_id.'"><span>'.$category_name.'</span></a></li>';
		$data['path_title']=$data['path_title'].'<li><a href="/gold/news_detail/'.$this->DataName.'/'.$category_id.'/'.$id.'"><span>'.$news['name'].'</span></a></li>';
		$data['banner']=$this->data['banner'];
	...
	public function about(){
		...
		$this->DataName='about';
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/gold/about"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
		$data['banner']=$this->data['banner'];

		$data['title'] = $this->lang["$this->DataName"];

		//撈取關於我們list
		$data['list']=$this->mymodel->get_about_data($this -> set_language);
		//view
		$this->load->view('index/header', $data);
		$this->load->view('index/about/about_nav', $data);
		$this->load->view('index/about/about', $data);
		$this->load->view('index/footer', $data);
	}
	
	public function about_show($s_id){
		@session_start();
		//撈取關於我們單筆資料
		$aboutdata=$this->mymodel->get_about_one_data($s_id,$this -> set_language);
		$data['about']=$aboutdata;

		$this->DataName='about';
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/gold/about"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
		$data['path_title']=$data['path_title'].'<li><a href="/gold/about_show/'.$s_id.'"><span>'.$aboutdata['name'].'</span></a></li>';
		$data['banner']=$this->data['banner'];
	...
	public function announce(){
		...
		//語言包
		$this->lang=$this->lmodel->config('37',$this->setlang);
	...
	public function service(){
		...
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/gold/service"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
		$data['banner']=$this->data['banner'];
		...
		$this->load->view('index/header', $data);
		$this->load->view('index/about/about_nav', $data);
		$this->load->view('index/service/service', $data);
		$this->load->view('index/footer', $data);
	}
	
	public function service_show($s_id){
		@session_start();
		//撈取服務專區單筆資料
		...
		$this->lang=$this->lmodel->config('1',$this->setlang);
		$data['path_title']='<li><a href="/gold/service"><span>'.$this->lang["$this->DataName"].'</span></a></li>';
		$data['path_title']=$data['path_title'].'<li><a href="/gold/service_show/'.$s_id.'"><span>'.$servicedata['name'].'</span></a></li>';
		$data['banner']=$this->data['banner'];

		//view
		$this->load->view('index/header', $data);
		$this->load->view('index/about/about_nav', $data);
		$this->load->view('index/service/service_show', $data);
		$this->load->view('index/footer', $data);
	}
	...
	public function forgot_set(){
		...
		if(!empty($dbdata)){
			if($dbdata['by_email']==Comment::SetValue('email')){
				...
				//主旨
				if($this->setlang=='TW')
					$subject='jcymall忘記密碼通知信';
				if($this->setlang=='ENG')
					$subject='jcymall Shop​ Forgot password notification letter';
				if($this->setlang=='JAP')
					$subject='「jcymall」は、パスワード通知の手紙を忘れました';
	...
	public function member_list(){
		...
		$dbdata=$this->mymodel->OneSearchSql('buyer','PID,d_is_member',array('by_id'=>$_SESSION['MT']['by_id']));
		
		if($dbdata['d_is_member']=='1'){
			$ddata=$this->mymodel->OneSearchSql('member','deadline',array('by_id'=>$_SESSION['MT']['by_id']));
			$line=round(($ddata['deadline'] - time()) / 86400);
			$data['line']=($line<=0)?0:$line;

			if($line<=30)
				$data['renewal']='1';
		}
		$data['IsMember']=0;
		if($dbdata['d_is_member']=='0' and $dbdata['PID']==$_SESSION['AT']['member_id']){
			$data['IsMember']=1;
		}

		$data['dbdata']=$dbdata;
		//view
		$this->load->view('gold/member_list', $data);
	}
	...
	public function renewal(){
		...
		if(!empty($_GET['mid'])){
			$member_id=base64_decode($_GET['mid']);

		}else{
			$member_id=$_POST['member_id'];
		}
		if($member_id!=''){
			$dbdata=$this->mymodel->OneSearchSql('member','deadline',array('member_id'=>$member_id));
			$line=round(($dbdata['deadline'] - time()) / 86400);
			if($line<=30){
				$renewal=strtotime(date('Y-m-d',$dbdata['deadline'])."+1 year");
				$this->mymodel->update_set('member','member_id',$member_id,array('deadline'=>$renewal));
				if(!empty($_GET['mid'])){
					echo "<script>alert('".$this->lang['renesu']."');window.close(); </script>";
					exit();
				}else
					echo $this->lang['renesu'];//續約成功

			}else{
				if(!empty($_GET['mid'])){
					echo "<script>alert('".$this->lang['opfailed']."');window.close(); </script>";
					exit();
				}else
					echo $this->lang['opfailed'];//續約成功
			}
		}
	...
	//會員基本資料
	public function member_info_ok(){
		$this->useful->iconfig();
		...
		$this->load->view('index/header', $data);
		$this->load->view('index/member/member_nav', $data);
		$this->load->view('index/member/member_info_ok', $data);
		$this->load->view('index/footer', $data);
	}
	*/
	...
	// 	$dbdata=$this->mymodel->SelectSearch('`order`','','id,by_id,order_id,(`total_price`+`lway_price`) as alltotal,details,FROM_UNIXTIME(`date`) as create_time',"WHERE `order_id` > '201704260005' and `status`=1 and `product_flow`=4 and d_status='N'");
		
	// 	foreach ($dbdata as $key => $value) {
	// 		$get_bonus=floor($value['alltotal']/100);
	// 		$ddata=$this->mymodel->WriteSQL('SELECT GROUP_CONCAT(`prd_name`) as prd_name FROM `order_details` where `oid`='.$value['id'].'',1);
			
	// 		$iaddata=array(
	// 			'OID'=>$value['id'],
	// 			'buyer_id'=>$value['by_id'],
	// 			'd_type'=>'19',
	// 			'd_val'=>$get_bonus,
	// 			'd_des'=>'訂單號碼 ['.$value['order_id'].'] - 商品：'.$ddata['prd_name'],
	// 			'is_send'=>'N',
	...
	// 	}
	// 	// $get_bonus=floor($_POST['alltotal']/100);
	...
	public function contact_ok(){
		...
		//view
		$this->load->view('index/header', $data);
		$this->load->view('gold/contact_ok', $data);
		$this->load->view('index/footer', $data);
	}
	...
	public function talklist(){
		...
		if($_SESSION['AT']['member_id']!=$_SESSION['MT']['member_id']){
			$this->useful->AlertPage('/gold/index/'.$_SESSION['AT']['acconut'].'','操作錯誤');
		}
		$this->load->model('gold_model',gmodel);

		$dbdata=$this->gmodel->get_talk_data($_SESSION['AT']['member_id']);
		foreach ($dbdata as $key => $value) {
			$bdata=$this->mymodel->OneSearchSql('buyer','name',array('by_id'=>$value['b_id']));
			$dbdata[$key]['name']=$bdata['name'];
		}	
		$data['dbdata']=$dbdata;
		//view
		$this->load->view('gold/talklist', $data);
	}
	//EBH留言內文
	public function talkapp($by_id=''){
		...
		if($_SESSION['MT']['by_id']!='' ){
			...
			if($by_id==''){
				...
				if($_SESSION['AT']['member_id']==$_SESSION['MT']['member_id']){
					$_SESSION['EBH']='1';
					$this->useful->AlertPage('/gold/talklist');
				}

				//左右側判斷
				$data['left']='M';
				$data['right']='B';
				$name=$this->mymodel->OneSearchSql('buyer','name',array('d_account'=>$_SESSION['AT']['account']));
				$data['name']=$name['name'];

				$data['dbdata']=$this->mymodel->select_page_form('talkapp','','d_type,d_content,create_time',array('b_id'=>$_SESSION['MT']['by_id'],'m_id'=>$_SESSION['AT']['member_id']));
			}else{
				//左右側判斷
				$data['left']='B';
				$data['right']='M';
				$name=$this->mymodel->OneSearchSql('buyer','name',array('by_id'=>$by_id));
				$data['name']=$name['name'];


				$data['b_id']=$by_id;
				$data['dbdata']=$this->mymodel->select_page_form('talkapp','','d_type,d_content,create_time',array('b_id'=>$by_id));
				$this->mymodel->update_set('talkapp','b_id',$by_id,array('d_read'=>'Y'));
			}
			//view
			$this->load->view('gold/talkapp', $data);
		}else
			$this->useful->AlertPage('/gold/login/talkapp',$this->lang['Login']);	//請先登入或註冊會員'
		@session_write_close();
	}
			...
			if($dbname=='buyer'){
				...
				if($id!=''){
					...
					if($data['is_member']=='Y'){
						...
						//去除陣列無用值
						$data=$this->useful->UnsetArray($data,array('cen_country','cen_city','cen_countory','cen_address','bank_name','bank_account_name','bank_account','is_member','shop_country','shop_city','shop_countory','shop_address'));
					...
					$member_data=$this->mymodel->select_page_form('buyer','','by_id',array('d_account'=>Comment::SetValue('d_account')));
					...
					if(Comment::SetValue('PID')!=""){
						...
						$data['d_dividend']=100;//接受推薦送100點紅利
						//推薦人也送100點紅利
