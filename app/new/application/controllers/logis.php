...
class Logis extends MY_Controller
{
	...
	public function regconfig($type=''){
		...
		if(!empty($_POST)){
			$this->mymodel->update_set('config','d_type',$_POST['save_type'],array('d_val'=>$_POST['d_send_content']));
			echo '<script>alert("儲存完畢");window.location.href="/logis/regconfig/'.$_POST['save_type'].'";</script>';
		}else{
			if($type=='service'){
			...
			//撈取資料
			$content=$this->mymodel->GetConfig($type);
