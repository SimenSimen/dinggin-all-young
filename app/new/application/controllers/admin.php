...
class Admin extends MY_Controller
{
		...
		else
		{
			...
			foreach($member as $key => $value)
			{
				...
				if($value['pem'] && $value['mobileprovision'])
				{
					...
					$data['ios_deadline'][$key] = $this -> get_deadtime($value['ios_createTime'], "+365day");
				}
			}
	...
	public function member_setting($account = '', $action = '')
	{
		...
		else
		{
			//data
			$data = $this -> data;
			// 20170627-form會有問題 所以額外處理
			if($account=='09602027881'){
				$account='0960202788';
			}

			$member = $this -> mod_admin -> select_from('member', array('account' => $account));
			// 20160715-特殊處理帳號-APPLE不讓我用Q~Q
			if($member['account']=='0960202788'){
				$member['account']=$member['account'].'1';
			}
