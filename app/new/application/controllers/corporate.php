...
class Corporate extends MY_Controller
{
	...
	public function main($type = '')
	{
		...
		if($type == '1' || $type == '2' || $type == '3'|| $type == '6')
			$data['texts'] = $texts = $this -> mod_cpr -> select_from_order_limit('ckeditor', array('ck_id', 'name', 'content'), array('type' => $type, 'enable' => '1', 'lang_type' => $this -> session -> userdata('lang')), 'sort', 'asc');
		else
			$data['texts'] = $texts = $this -> mod_cpr -> inner_join_order_by('ckeditor', 'auth_category',array('ckeditor.*', 'auth_category.c_name'), array('ckeditor.type' => $type, 'ckeditor.enable' => '1', 'ckeditor.lang_type' =>  $this -> session -> userdata('lang'), 'auth_category.lang_type' => $this -> session -> userdata('lang')), 'category_id', 'ckeditor.sort', 'asc', 'array');
	...
	public function ckeditor_del()
	{
		...
		if(is_array($ck_array) && is_array($del_array))
		{
			...
			{
				$update_id[] = $this -> mod_cpr -> update_set('ckeditor', array('ck_id' => $value), array('enable' => '0'));
			}
