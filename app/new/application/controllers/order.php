...
class Order extends MY_Controller
{
	...
	public function __construct()//初始化
	{
        ...
        @session_start();
		$this->load->model(array('banner_model', 'products_model', 'order_model'));
	...
	public function cancel($id=''){		
		...
		{				
			...
			if($orderdata['status']<>0 or $orderdata['product_flow']<>0){	//此訂單不能取消
	...
	public function refund($id=''){			
		...
		{				
			...
			if($orderdata['status']<>1 or $orderdata['product_flow']<>4 or strtotime($today)>strtotime($orderdata['deadline'])){
				$this->useful->AlertPage('/order');	//此訂單不能退貨
			}
			$data['path_title']='<li><a href="/gold/member"><span>'.$this->lang['member'].'</span></a></li>'.
			 					'<li><a href="/'.$this->DataName.'"><span>'.$this->lang["$this->DataName"].'</span></a></li>'.
			 					'<li><a href="/'.$this->DataName.'/refund/'.$id.'"><span>申請退貨:'.$orderdata['order_id'].'</span></a></li>';
			$this->load->view('index/header', $data);
			$this->load->view('index/member/member_nav', $data);
	...
	public function member_sale($cset_code='')
	{
		...
		if($_SESSION['MT']['is_login']==1){	// 判斷是否登入
			if($_SESSION['MT']['d_is_member']==1){		//	判斷是否為經營者
				...
				$data['path_title']='<li><a href="/gold/member"><span>'.$this->lang['member'].'</span></a></li>'.
									'<li><a href="/'.$this->DataName.'/member_sale"><span>'.$this->lang['member_sale'].'</span></a></li>';
				//推薦人
				$by_id = $_SESSION['MT']['by_id'];
				$buyer = $this->mymodel->OneSearchSql('buyer','PID, d_dividend',array('by_id'=>$by_id));
				$data['d_dividend']	= $buyer['d_dividend'];
	...
	public function order_bonus(){
		...
		foreach ($data as $value) {
			$this->mymodel->update_set('`dividend`','OID',$value['id'],array('is_send'=>'Y'));
	...
	public function order_supplier(){//銷貨(供應商)報表20171228
		@session_start();
		//權限判斷
		$this->useful->CheckComp('j_orderinfo');
		//資料庫名稱
		...
		//分頁程式 start
		$data['ToPage']=$Topage=!empty($_POST['ToPage'])?$_POST['ToPage']:1;		
		$sql="(select sum(number) as number from order_details as o inner join supplier s on s.d_id=o.supplier_id ".$where." group by prd_id,price order by supplier_id ) a";		
		$qpage=$this->useful->SetPage($sql,'',20);
		$data['page']=$this->useful->get_page($qpage);
		//分頁程式 end
		//訂單資料
		...
		//撈取供應商名稱
		$data['supplier']=$supplier=$this->mymodel->select_page_form('supplier','','d_id, d_name',array());
		//view
	...
	public function order_back_list(){//退貨單明細列表
		...
		$this->omodel->search_session($search_default_array);		
		$where_array=array();
		$where_array[]="product_flow in(3,5)";
		if($_SESSION["AT"]["where"]["txt"]!=""){
		...
		/*
		if($_SESSION["AT"]["where"]["date_start"]!=""){
			$where_array[]="back_date>='".strtotime($_SESSION["AT"]["where"]["date_start"])."'";
		}
		if($_SESSION["AT"]["where"]["date_end"]!=""){
			$where_array[]="back_date<='".strtotime($_SESSION["AT"]["where"]["date_end"])."'";
		}
		*/
		if($_SESSION["AT"]["where"]["date_start"]!=""){
			$where_array[]="create_time between'".$_SESSION["AT"]["where"]["date_start"]."  00:00:00' and '".$_SESSION["AT"]["where"]["date_end"]." 23:59:59'";
	...
	public function order_list(){//訂購單明細列表
		...
		$sql="(select o.* from `order` as o inner join warehouse w on w.d_id=o.warehouse_id ".$where." order by o.id ) a";
		$qpage=$this->useful->SetPage($sql,'',20);
		$data['page']=$this->useful->get_page($qpage);
		//分頁程式 end
		//訂單資料
	...
	public function order_invoice_print(){//發票列印
		...
		//view
		$this->load->view('order/order_invoice', $data);
	}
