...
<head>
  ...
  <!-- seo -->
  <title>風格設定 - <?=$web_config['title']?></title>
...
<div id="container"><div class="w1024">
  ...
  <h1 style="font-size:1.09em;">請選擇您喜歡的行動商務系統版型：點擊風格名稱右方連結可前往預覽視窗</h1>
  <h1>&nbsp;</h1>
          ...
          <?php foreach ($theme as $key => $value): ?>
            ...
            <li class="ui-state-default">
              ...
              <span class='theme_display_name'><?=$value['theme_display_name']?>&nbsp;
                <a onclick='pop_theme_preview("<?=$base_url.$value['theme_preview_link']?>", "<?=$value['theme_display_name']?>");' class='theme_preview aa5' title='點此預覽'>預覽</a>
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="table-theme">
        ...
        <!--按鈕顏色-->
          <tr id='jqm_button_select_div' style="display:none;">
            <td class="dd1">按鈕顏色</td>
            <td class="dd2">
              <select name='jqm_button_color' id='jqm_button_color'>
              <option value='a' <?=$jqm_button_selected['a']?>>黑色</option>
              <option value='b' <?=$jqm_button_selected['b']?>>藍色</option>
              <option value='c' <?=$jqm_button_selected['c']?>>灰色</option>
              <option value='d' <?=$jqm_button_selected['d']?>>白色</option>
              <option value='e' <?=$jqm_button_selected['e']?>>黃色</option>
              </select>
            </td>
          </tr>
...
?>	
        ...
        <!--背景顏色-->
          <tr>
            <td class="dd1">背景設定</td>
            <td class="dd2">
              <input type='radio' name='background_type' class='switch_background_type' id='color_background' <?=$bg_type_checked[0]?> value='0'><label class='switch_background_label' for='color_background'>使用顏色</label>&nbsp;
              <input type='radio' name='background_type' class='switch_background_type' id='image_background' <?=$bg_type_checked[1]?> value='1'><label class='switch_background_label' for='image_background'>使用素材</label>
            </td>
          </tr>

        <!--背景顏色-->   
          <tr>
            <td class="dd1">背景顏色</td>
            <td class="dd2">
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="table-theme">
        <?php if ($dfu_theme['dfu_bg_type'] != 0): ?><!--預設為圖形模式，顯示預設背景欄位-->
          <tr class='theme_background_tr' style="display:none;" id='image_background_tr'>
            ...
            <td width="100" class="font_crm_list">背景素材：</td>
            <td style="text-align:left;" class="font_crm_list">
              ...
              <label for='theme_background_radio_basic' class='theme_background_label'>使用版型預設背景圖片</label>
              <img id="help1" src="/images/front/help1.gif" title="您可以選擇使用預設背景，或系統提供的素材作為行動商務系統背景" />
    ...
    <!--儲存按鈕-->
    <div class="aaCenter">
      <input type='button' class='aa3' id='reset_style' value='恢復系統預設值'>
      <input type='button' class='aa3' id='reset_edit' value='重新設定'>
      <input type='submit' class="aa3" name='form_submit' onclick="window.onbeforeunload=null;return true;" value='儲存'>
    </div>
