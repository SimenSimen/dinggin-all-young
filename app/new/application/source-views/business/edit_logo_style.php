...
<head>
  ...
  <!-- seo -->
  <title>封面照設定 - <?=$web_config['title']?></title>
...
<div id="con-L">
  <div style="width:300px; height:240px; margin-bottom: 40px; padding: 10px; background: #333;">
    <?php if(!empty($iqr['logo_path'])):?>
      <input style="position: absolute; padding: 0 5px; margin-left: 260px;" type="button" id="del_image" value="刪除">
      <img style="border-radius: 10px;" class="show_img" src="/<?=$iqr['logo_path']?>">
      <?php else: ?>
      <span id="sp" style="text-align: center; color: white; vertical-align: center; position: absolute; margin: 120px;">請上傳圖片</span>
    <?php endif ;?>
  </div>
  ...
  <?php if(empty($iqr['logo_path'])):?>
    <form id='f' name="form_submit" action="/business/logo" method="post" enctype="multipart/form-data">
      ...
      <input style="padding: 0 10px;" type="submit" id='e' value="送出">
    </form>
  <?php endif; ?>
</div>
