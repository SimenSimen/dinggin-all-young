...
<head>
  ...
  <!-- seo -->
  <title>行動商務系統快速連結 - <?=$web_config['title']?></title>
    ...
    $(function(){
      $('#text').click(function(){
        <?php if ($device_check == 1): ?>
          window.open('<?=$base_url?>business/iqr/<?=$account?>', '<?=$l_name.$f_name?>的行動商務系統', config='scrollbars=1,outerWidth=300,innerWidth=300,height=640,width=300,left=900,scrollbar=yes');
        <?php else: ?>
          window.open('<?=$base_url?>business/iqr_html/<?=$account?>', '<?=$l_name.$f_name?>的行動商務系統', config='scrollbars=1,outerWidth=300,innerWidth=300,height=640,width=300,left=900,scrollbar=yes');
        <?php endif; ?>
      });
    });
  </script>
...
<span style="font-family:'微軟正黑體';font-size:1.05em;"><?=$qrcode_box_text?><br><br></span>
    ...
    <a style='cursor: pointer;' title='點此下載, IE用戶請使用右鍵→另存圖片' id="download" download="<?=$l_name.$f_name?>的行動商務系統.png">
...
<?php if ($edit_target != 2): ?>
  <!-- qrcode url -->
  <textarea readonly="true" style="resize:none;width:85%;cursor: pointer;height:62.8px;" name="iqr_url" id="text">※ 您也可以點此直接開啟 <?=$iqr_url?></textarea>
