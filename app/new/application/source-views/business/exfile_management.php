...
<head>
  ...
  <!-- seo -->
  <title>附件管理 - <?=$web_config['title']?></title>
...
<!--左-->
  <div id="con-L" style="height: 1332px;">
   ...
   <div class="step-docUP-1">
      ...
      <?php if ($exfile_num == 0): ?>
        <h2 class="boxs_title" id='exfile_boxs_title'>目前沒有任何附件</h2>
      <?php else: ?>
        <h2 class="boxs_title" id='exfile_boxs_title'>您目前有<?=$exfile_num?>個附件</h2>
      <?php endif; ?>
      <input type='button' class="aa2" id='add_exfile' value='+ 新增附件'><span class='introdution_prompt' id='msg'><?=$msg?></span>
          ...
          <?php if (!empty($exfile)): ?>
            <table class='exfile_management_table'>
              <tr>
                ...
                <td>原始檔名</td>
                <td>按鈕名稱</td>
                <td style="text-align:center;">下載</td>
              </tr>
              <?php foreach ($exfile as $key => $value): ?>
                  <tr>
                    ...
                    <td style="text-align:center;"><a class='aa7' target="_blank" href='<?=$value?>'>下載</a></td><!-- download='<?=$doc_ori_name[$key]?>' download='<?=$exfile_name[$key]?>'-->
                  </tr>
              <?php endforeach; ?>
            </table>
          <?php endif; ?>
              ...
              <tbody>
                <?php foreach ($exfile as $key => $value): ?>
                    <tr>
                      ...
                      <td><div class='exfile_ori_name'><?=$exfile_ori_name[$key]?></div></td>
                      <td>
                        <input type='text' class='iii2' placeholder='提醒您，字數限制15字' style="width:75%;" maxlength="15" name='exfile_name[<?=$exfile_id[$key]?>]' value='<?=$exfile_name[$key]?>'>
                        <a href="#" class="why">?</a>
        ...
        <div class='switch_exfile' id='exfile_delete' style="display:none;">
          ...
          <?echo form_open("/business/exfile_remove", array('name'=>'form_exfile_remove', 'id'=>'form_exfile_remove'));?>
            <table class='exfile_management_table' id='remove_table'>
              <tr>
                <td><input type="checkbox" name='clickAll' id='clickAll' />&nbsp;全選</td>
                <td>原始檔名</td>
                <td>按鈕名稱</td>
              </tr>
              <?php foreach ($exfile as $key => $value): ?>
    ...
    <div class="aaCenter">
      <div class="bottom_button" id='exfile_bottom_button'>
        <input type='button' class='aa6' id='exfile_sort_btn' value='排序附件'>
        <input type='button' class='aa6' id='exfile_edit_btn' value='編輯按鈕名稱'>
        <input type='button' class='aa6' id='exfile_del_btn' value='移除附件'>
      </div>
      <div class="bottom_button" id='exfile_sort_button' style="display:none;">
          <input type='button' class='aa6' id='exfile_sort_submit' value='儲存附件排序'>
          <input type='button' class='aa6 exfile_cancel_btn' value='取消'>
      </div>
      <div class="bottom_button" id='exfile_edit_note_button' style="display:none;">
          <input type='button' class='aa6' id='exfile_edit_note_submit' value='儲存附件按鈕名稱'>
          <input type='button' class='aa6 exfile_cancel_btn' value='取消'>
      </div>
      <div class="bottom_button" id='exfile_remove_button' style="display:none;">
          <input type='button' class='aa6' id='exfile_remove_submit' value='移除所選附件'>
          <input type='button' class='aa6 exfile_cancel_btn' value='取消'>
      </div>
    </div>
