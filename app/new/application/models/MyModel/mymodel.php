...
class Mymodel extends MY_Model{
	...
	public function get_address_one_data($s_d_id='0'){
		$sql="SELECT d_id,name,telphone,country,city,countory,address FROM address where ";
		$sql.=" d_id=".$s_d_id;
		$query = $this->db->query($sql);
		return $query->row_array();
	}
	...
	public function productsType(){//產品分類		
    	$sql="SELECT `prd_cid`, `prd_cname`, `PID` FROM `product_class` where lang_type ='$this->setlang' and PID = 0 order by `prd_cid`";
    	$query = $this->db->query($sql);
		$data['product_type1']=$query->result();		
    	$sql2="SELECT `prd_cid`, `prd_cname`, `PID` FROM `product_class` where lang_type ='$this->setlang' and PID <> 0 order by `prd_cid`";
