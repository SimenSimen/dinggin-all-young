    ...
    <?php if (!empty($dbdata)): ?>
        ...
        <?php foreach ($dbdata as $key => $value): ?>
          <tr bgcolor='<?=$member_auth_color[$key]?>'>
            <td class='center_td white_td'><input type="checkbox"  name="allid[]" value="<?=$value['lway_id']?>"></td>
            <td class='center_td white_td'><?=$value['active']?></td>
            <td class='center_td white_td'><?=$value['lway_name']?></td>
            <td class='center_td white_td'><?=$value['business_account']?></td>
            <td class='center_td white_td'> 
              <a href="javascript:void(0);" onclick="top.frames['content-frame'].location='/<?=$dbname?>/<?=$dbname?>_info/<?=$value['lway_id']?>'">
...
<script>
  function allcheck(){
    var str=''; 
    var DB='logistics_way';     //資料庫
    var Field='lway_id'; //欄位名稱
