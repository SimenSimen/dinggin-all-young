...
<html lang="en" class="no-js">
  <head>
      ...
      <title><?=$this->lang1['sproduct'];//商品搜尋?> - <?=$web_config['title']?></title>
  ...
  <body>
   ...
   <header>
   	 <div class="www">
           ...
           <div class="col-xs-8"><h1><?=$this->lang1['strsearch'];//搜尋?></h1></div>
    ...
    <div class="main">
        ...
        <section class="search">
           <div class="row">
               ...
               <?php if($tips == 'fail'):?>
                   <p class="ff2"><span class="icon-notice"></span><?=$this->lang1['keyword'];//關鍵字?><?=$search_key?>&nbsp;&nbsp; <?=$this->lang1['nopro'];//無相關產品，請重新搜尋?></p><br>     
                <?php endif;?>
                  <form id="search" method="post" action="/cart/search_engine/<?=$cset_code?>">
                  <input name="search_key" type="text" class="form-control input" placeholder="<?=$this->lang1['ikeyword'];//請輸入關鍵字?>">    
                  <a class="aa6" href="javascript:void(0);"><span class="icon-search"></span><?=$this->lang1['strsearch'];//搜尋?></a>         
                  </form>
               </div>
            </div> 
        </section>   
        ...
        <?php if($search_key && $tips != 'fail'):?>
            <h3><?=$this->lang1['keyword'];//關鍵字?>：<?=$search_key?></h3>
        <?php endif; ?>
               ...
               <div class="col-sm-2 col-xs-4">
                 <div class="proView">
                   <a href="/cart/product_info/<?=$cset_code?>/<?=$value['prd_id']?>">
                    <div class="centerIMG"><img src="<?=$img_url.'set_'.substr($value['prd_image'], 1)?>" alt=""/></div>
