...
<head>
  ...
  <!-- seo -->
  <title><?=$_EditCustomPages?> <?=$web_config['title']?></title>
...
<body scroll="yes" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="text-align: center; overflow-x: hidden;overflow: -moz-scrollbars-vertical;">
  ...
  <div>
    ...
    <!--編輯自訂網頁form-->
    <form action='/business/iqr_html_edit' method='post' name='form_iqr_html_edit' id='form_iqr_html_edit'>
      <h3 style="font-family: '微軟正黑體';"><?=$EditCustomPages?></h3>
        <table class='table iqr_html_add_table' id='iqr_html_add_table' style="width:90%;">
          <tr>
            ...
            <td>
              ...
              <?php foreach ($classify as $key => $value): ?>
                <option value="<?=$value['classify_id']?>" <?php if($iqr_html['classify_id'] == $value['classify_id']):?>selected<?php endif;?>><?=$value['classify_name']?></option>
              <?php endforeach ?>
              </select>
          ...
          <tr>
            ...
            <td class='table-cell-left'><input type='text' class='form-control required' name='iqr_html_name' id='iqr_html_name' maxlength="64" value='<?=$iqr_html['html_name']?>' placeholder='<?=$Max64fonts?>'></td>
          </tr>

          <tr>
            <td class='table-cell-right'><?=$WebContent?></td>
            <td class='table-cell-left'><textarea name='iqr_html_content' id='iqr_html_content'><?=$iqr_html['html_content']?></textarea><p style="color:red;" id='iqr_html_prompt'></p></td>
          </tr>

          <tr>
            <td class='table-cell-center' colspan="2" style="text-align:center;">
              <input type='submit' class='btn btn-default' name='form_submit' value='<?=$Save?>' onclick="window.onbeforeunload=null;return true;">
...
<!--bottom script-->
<script type="text/javascript" src="/js/iqr_html_edit.js"></script>