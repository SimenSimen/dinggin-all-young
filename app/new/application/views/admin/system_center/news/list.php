	...
	<script>
		$(function() {
			...
			$('.btn-info').click(function() {
				$('#list_form').attr('action', '/news/sort_save');
				$('#list_form').submit();
			});

			$('.btn-danger').click(function() {
				$('#list_form').attr('action', '/news/form_del');
				$('#list_form').submit();
			})
			...
			$('.trash_group').click(function() {
				$.post('/news/del_group', { id: $(this).attr('id') },
					function(response) {
...
<body bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="text-align: center">
  	...
  	<table id='button_table'>
	    <tr style="text-align:right;">
	      ...
	      <?php if($news_btn): ?>
	      	<td style="width:1%;"><button class="btn btn-primary" type="button" onclick="window.open('/news/enews/add', '新增最新消息', config='height=500, width=700')">新增最新消息</button></td>
	      <?php endif; ?>
		...
		<?php if(!empty($group)): ?>
			<?php foreach ($group as $key => $value): ?>
				<tr>
					...
					<td><?=$value['c_name']?></td>
					<td>
						<a href="/news/edit/g/<?=$value['category_id']?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
		...
		<form id="list_form" method="post">
			<table id='list' class='table table-hover table-bordered table-condensed' style="width:80%;">
				...
				<?php foreach ($news as $key => $value): ?>
				    ...
				    <tr>
				    	<input type="hidden" name="ck_id[]" value="<?=$value['enews_id']?>">
				    	<td><input type="checkbox" name="check_id[]" value="<?=$value['enews_id']?>"></td>
				    	<td><?=($key+1)?></td>
				    	<td><?=$value['c_name']?></td>
				    	<td><?=$value['name']?></td>
				    	<td>
				    		<a onclick="window.open('/news/my_enews/<?=$value['member_id']?>/<?=$value['enews_id']?>', '<?=$value['name']?>', config='height=500,width=700');"><i class="fa fa-file-text-o" aria-hidden="true"></i></a>
				    		<a onclick="window.open('/news/enews/edit/0/<?=$value['enews_id']?>', '<?=$value['name']?>', config='height=500,width=700');"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
							<a href="/news/enews/delete/0/<?=$value['enews_id']?>" id=""><i class="fa fa-trash-o" aria-hidden="true"></i></a>
			    		</td>
				    </tr>
				<?php endforeach; ?>
				</tbody>
