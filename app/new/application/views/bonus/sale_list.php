        ...
        <?php foreach ($dbdata as $key => $value): ?>
          <tr bgcolor='<?//=$member_auth_color[$key]?>'>
            ...
            <td class='center_td white_td'> 
              <a href="javascript:void(0);" onclick="top.frames['content-frame'].location='/bonus/sale_info/<?=$value['id']?>'">
...
$(function() {
  ...
  $("#search_action").click(function(){
    $("#search_form").attr('action','/bonus/sale_list/<?php echo isset($_SESSION["AT"]["where"]["by_id"])?$_SESSION["AT"]["where"]["by_id"]:"";?>');
    $("#search_form").submit();
  });
  $("#goback_action").click(function(){
    $("#search_form").attr('action','/bonus/sale');
    $("#search_form").submit();
  });
});
</script>